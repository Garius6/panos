...
mod style) — заведена, открытые design-вопросы не решены. **Приоритет
(grilled)**: Стадия 22 следующая, 23/24/25/26/4/8 — после её закрытия,
не параллельно.
...
### Стадия 26 (заведено, не исследовано) — `panos mod`: встроенный пакетный менеджер

**Что**: подкоманды у самого бинарника `panos` (`panos mod init`/
`panos get`/аналоги) в духе `go mod` — ОДИН бинарник, не отдельный
инструмент. Решено явно (см. TASKS.md): существующий заготовочный
проект `pan` (`/Users/gaidar/dev/panosiki/pan/` — README + demo
flag-parser, 0 коммитов) архитектурно не подходит под это решение
(отдельный бинарник, cargo-стиль) — становится неактуальным, не
трогается (не удаляется/не архивируется), просто план живёт здесь, не
там.
...
- `модули/`-fallback уже в резолвере (`resolver_import_native.odin:12`,
  ...
  туда работает БЕЗ единой правки в резолвере, инфраструктура импорта
  уже готова принимать скачанные пакеты.
- `std/кодирование/toml.ps` — реальный TOML-парсер уже написан на
  panos — манифест-файл может использовать существующий формат/парсер
  вместо нового.

**Не решено (открытые вопросы, нужно обсуждение/грилинг перед
Порядком работ)**:
- **Разрешение версий**: полноценный Minimal Version Selection
  (Go-style — для каждой зависимости берётся МИНИМАЛЬНАЯ версия,
  удовлетворяющая всем требованиям графа, детерминированно, без
  SAT-solver'а) — реальная сложность (diamond dependencies,
  транзитивные конфликты) — или для v1 сильно проще: плоский список
  прямых зависимостей БЕЗ транзитивного резолва вообще (каждый проект
  явно объявляет ВСЕ свои зависимости, никакой "зависимости
  зависимостей")?
- **Источник пакетов**: только git (clone/checkout по тегу), без
  своего registry/индекс-сервера — имя пакета = git URL напрямую (как
  ранний Go до GOPROXY)?
- **Схема версионирования**: semver git-теги (`v1.2.3`) — с
  Go-style конвенцией "major-версия в пути импорта" (избегает
  diamond-проблем для v2+) или без неё (проще, но конфликт двух
  несовместимых мажорных версий одной зависимости не разрешится)?
- **Формат имён файлов манифеста/lock**: `панос.toml`/`панос.lock`
  (кириллица, консистентно с языком) или `panos.toml`/`panos.lock`
  (латиница, консистентно с внешней конвенцией — `go.mod`,
  `Cargo.toml`, `package.json` все латиницей)?
- **Кто парсит манифест**: сам Odin-хост (нативно, до старта VM — проще
  для bootstrap, не поднимать VM только чтобы прочитать манифест перед
  сборкой) или panos-код (`std/кодирование/toml.ps`, вызванный из
  хоста)?

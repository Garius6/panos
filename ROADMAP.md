...
Стадия 23-Арифметика ✅, 23-Печатаемое ✅, 23-Копируемое ✅ закрыты.
По-умолчанию выброшена из плана (мотивация была спекулятивной,
реального consumer'а не нашлось — см. §3). Остаток Стадии 23
(Хешируемое/Итерируемое) — заводятся СВОИМИ стадиями (уже решено при
исходном грилинге, не довесок к 23). Итерируемое исследована
(iterator protocol выбран grilling'ом, for-in архитектурный барьер
найден и решение согласовано) и ОБНАРУЖИЛА жёсткий prerequisite —
Стадию 28 (generic-интерфейсы, не исследована). Хешируемое не
исследована.
...
binding-immutability, НЕ deep immutability) ✅ закрыта. Стадия 28
(generic-интерфейсы — недостающая под-фаза Стадии 7, найдена как
prerequisite Итерируемого) — найдены touch-точки беглым просмотром, НЕ
formal Explore, объём не оценён.
**Приоритет (grilled)**: 28→23-Итерируемое, 24/25/26/4/8 — следующие
кандидаты, не параллельно.
...
- **Хешируемое (Hash)** — тир: тяжёлый, отдельная подсистема. Нужен для
  ...
  investigation-скоуп.
- **Итерируемое (Iterable)** — тир: тяжёлый, архитектурный,
  **ЗАБЛОКИРОВАНА Стадией 28** (generic-интерфейсы — жёсткий
  prerequisite, найден при investigation, см. §3 Стадия 28 ниже).
  Grilled: iterator protocol (`итератор()`/`следующий() -> Опция(T)`,
  Rust/Python-style), НЕ indexable protocol (`длина()`+`[idx]`) — общее
  для ленивых/бесконечных последовательностей, не навязывает random-
  access форму. `для x в` сейчас чистый parser-level sugar (Стадия 17,
  десахаривается ЦЕЛИКОМ на этапе парсинга, до всякой типизации,
  жёстко под `.длина()`+`[idx]` — см. parser.odin:1176-1295) —
  найдено при Explore-investigation (см. TASKS.md): iterator-protocol
  форма (`выбор ...следующий() { Есть(x) -> ...; Нет -> прервать }`)
  структурно НЕСОВМЕСТИМА с уже десахаренными узлами (другой control
  flow — match, не сравнение; байндинг из деструктуризации варианта, не
  индексации) — нельзя просто переиспользовать Call_Info-sugar
  (проверенный 5 раз в этой сессии механизм) на тех же AST-узлах.
  Решение: НЕ десахаривать for-in на этапе парсинга целиком — завести
  необесахаренный `For_In_Stmt` (паттерн/`в`-выражение/тело как есть),
  резолвить/типизировать обычным образом, а выбор формы (fast-path для
  Массив/Соответствие/Строка vs iterator-protocol) и генерацию байткода
  делать в compiler.odin, где уже есть полный тип. Требует Стадии 28
  ПЕРЕД этим — `следующий() -> Опция(T)` нуждается в T как независимом
  параметре интерфейса, чего сейчас нет вообще.
...
### Стадия 28 (найдена при investigation Итерируемого, не исследована) — generic-интерфейсы

**Статус**: обнаружена как ЖЁСТКИЙ ПРЕРЕКВИЗИТ для Итерируемого (Стадия
23), не самостоятельная инициатива. По сути — недостающая под-фаза уже
в основном реализованной Стадии 7 (Generics: generic-структуры/enum'ы/
функции готовы, generic-интерфейсов нет вообще), не новый домен
возможностей с нуля. Beглый просмотр (не formal Explore) нащупал
конкретные touch-точки, но объём/риски НЕ оценены — нужен полноценный
Explore перед Порядком работ, как и у прочих "тяжёлых" стадий (24/25/26).

**Что**: `тип Итерируемое[T] = интерфейс ... конец` — интерфейсы с
type-параметрами, инстанциируемые с явным типом-аргументом
(`Итерируемое(Число)`) и в `реализация`-блоках, и как аннотация типа
параметра. Сейчас `Interface_Decl` (parser.odin:78-83) физически не
имеет `type_params` (в отличие от `Struct_Decl`/`Enum_Decl`, у которых
есть) — ни один из 6 интерфейсов, сделанных в этой сессии (Сравниваемое/
Равнозначное/4×Арифметика/Печатаемое/Копируемое), не нуждался в
постороннем типе-параметре (методы возвращают конкретный тип или Self
через уже существующий фикс) — Итерируемое первый случай, где нужен
НЕЗАВИСИМЫЙ параметр (T в `следующий() -> Опция(T)`), Self тут не
помогает.

**Найдено беглым просмотром**:
- `core/parser.odin:78-83` (`Interface_Decl`) — нет `type_params`.
  `core/parser.odin:1011-1013` (`parse_struct_decl`) уже вызывает
  переиспользуемый `parse_type_params(p)` после имени — тот же вызов
  добавляется в `parse_interface_decl` (parser.odin:770+) после чтения
  имени, симметрично.
- `core/type_cheker.odin:1351-1356` (ПРОХОД 1, `case ^Interface_Decl`)
  — создаёт `Type{kind = .Interface}` БЕЗ `generic_origin` (в отличие
  от Struct_Decl/Enum_Decl рядом, у которых `generic_origin = sym`
  выставлен) — нужно выставлять симметрично.
- `core/type_cheker.odin:1446-1455` (ПРОХОД 2, `case ^Interface_Decl`)
  — резолвит `interface_methods` БЕЗ обёртки в
  `if len(d.type_params) > 0 { make_decl_type_params(...); ...;
  try_generalize(...) } else { ... }`, которую уже используют
  Struct_Decl (type_cheker.odin:1396-1420) и Enum_Decl
  (type_cheker.odin:1468+) — тот же паттерн, симметрично применим (в
  принципе — `resolve_type_node` внутри `interface_method_type_from_
  signature` уже ищет имена в `ctx.current_type_params`, тот же
  механизм, что и у полей структуры).
- **Не проверено/не найдено беглым просмотром** (нужен Explore):
  что даёт `try_generalize` применительно к ИНТЕРФЕЙСУ — у Struct_Decl/
  Enum_Decl полиморфная схема нужна для ИНСТАНЦИАЦИИ ПРИ ВЫЗОВЕ
  конструктора (`Коробка(5)` — call-site type inference); интерфейс
  никогда не "вызывается" как конструктор — использование иное: (а) как
  target `реализация X(Тип-аргумент) для Y`, (б) как аннотация типа
  параметра `функ f(x: Итерируемое(Число))`. Нужно исследовать, поддер-
  живает ли `parse_impl_decl` (сейчас — вероятно, только голый
  идентификатор имени интерфейса) синтаксис `Итерируемое(Число)` с
  явным типом-аргументом вообще, и как резолвится инстанциация в обоих
  контекстах (а)/(б) — вероятно НЕ тот же путь, что call-site inference
  конструкторов, нужен отдельный механизм явной подстановки (ближе к
  тому, как `Пара(Число, Строка)`-аннотация типа уже резолвится сегодня
  для generic-структур в позиции типа, не в позиции вызова).

**Prerequisite**: Стадия 7 (Generics) — фактически уже готова (структуры/
enum'ы/функции), эта стадия — недостающий кусок её же охвата.

**Effort**: не оценено — беглый просмотр нашёл symmetric touch-точки
на уровне parser/ПРОХОД 1, но instantiation-семантика интерфейсов
(отличная от конструкторов) не исследована вообще — реальный объём
может быть больше, чем кажется по трём найденным точкам.

...
умолчанию) ✅, 28 (generic-интерфейсы) ✅.
...
prerequisite Итерируемого) ✅ закрыта, оказалась проще исходной оценки.
23-Итерируемое теперь РАЗБЛОКИРОВАНА, но ещё не реализована (нужен
`For_In_Stmt`-рефакторинг for-in, см. §3).
**Приоритет (grilled)**: 23-Итерируемое, 24/25/26/4/8 — следующие
...
- **Хешируемое (Hash)** — тир: тяжёлый, отдельная подсистема. Нужен для
  ...
  investigation-скоуп.
- **Итерируемое (Iterable)** — тир: тяжёлый, архитектурный. Стадия 28
  (generic-интерфейсы — был жёсткий prerequisite) ✅ ЗАКРЫТА — Итерируемое
  разблокирована, но своя реализация (`For_In_Stmt`-рефакторинг, см.
  ниже) ещё НЕ начата. Grilled: iterator protocol (`итератор()`/
  `следующий() -> Опция(T)`,

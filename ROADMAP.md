...
владения — `-> Указатель(Целое) владеет_я`/`владеет_C` (default —
`владеет_C`, безопасный дефолт: никогда не освобождать чужую память) —
ни один из идентификаторов (`Целое`/`КСтрока`/`Указатель`/`владеет_я`/
`владеет_C`) не стал новым keyword'ом лексера, распознаются строковым
...
`владеет_я` — не падает, `getenv` без владения — не пытается

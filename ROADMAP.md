...
Пусто-функции, LSP crash-фиксы, for-in диагностика. Стадия 22
(Сравниваемое/Равнозначное — operator sugar) — распланирована, не начата.
Стадия 23 (Печатаемое/Арифметика/По-умолчанию/Копируемое/Хешируемое/
Итерируемое) — список кандидатов, требует investigation до плана.
Стадия 24 (stackful coroutines) — ключевые решения прошли grilling и
подтверждены (см. §3 ниже), touch-точки реализации требуют Explore-
investigation до плана. Стадия 25 (интерфейсы для перечислений) —
untracked gap, обнаружен при grilling Стадии 22, независим, не
исследован. **Приоритет (grilled)**: Стадия 22 следующая, 23/24/25/4/8
— после её закрытия, не параллельно.
...
- **Печатаемое (Show/Display)** — `вСтроку() -> Строка`. Уже
  используется как ГОЛЫЙ ПРИМЕР синтаксиса интерфейсов в
  docs/generics.md (не prelude, не реальный type class). Тир: лёгкий —
  та же Self-тип+prelude-интерфейс инфраструктура, что Стадия 22, БЕЗ
  operator-sugar части (не бинарный оператор — нужен только hook в месте,
  где значения форматируются в строку, напр. `вывод.печать`/строковая
  интерполяция: если аргумент реализует Печатаемое — звать `.вСтроку()`
  вместо дефолтного `%v`-дампа полей). Не исследовано: где именно живёт
  эта точка форматирования в vm.odin — нужен Explore перед планом.
- **Арифметика** (`+`/`-`/`*`/`/` overload) — тир: лёгкий, буквально та
  же механика, что Стадия 22 (Self-тип + `Binary_Expr`-sugar), просто
  под `.Plus`/`.Minus`/`.Star`/`.Slash`. Осложнение, которого не было у
  Ord: `.Plus` УЖЕ хардкод-диспатчит Число+Число ИЛИ Строка+Строка
  (конкатенация) в одном кейсе (type_cheker.odin ~2555, до кейса
  Less/Greater) — sugar-ветка должна аккуратно встать ПОСЛЕ обеих
  существующих хардкод-проверок, не конфликтовать со строковой
  конкатенацией.
- **По-умолчанию (Default)** — `тип() -> Тип`, ноль параметров. Тир:
  лёгкий-средний — ПРОЩЕ Ord (нет Self-параметра/двух операндов, обычный
  интерфейс с одним методом без получателя-от-другого). Даёт
  `Массив.заполнить(N)` и подобным не требовать явного значения, если
  элемент реализует По-умолчанию — сам hook-up (где заполнение массива
  сейчас требует явного аргумента) не инвентaризирован.
- **Копируемое (Clone)** — семантика решена (grilled): структуры в
  panos — reference semantics по умолчанию (см. §8 "Важный факт о
  текущей семантике"), присваивание копирует указатель, не поля.
  Копируемое — единственный способ получить независимую копию, не
  нишевый кейс "вложенный Array/Map". Решено: ГЛУБОКАЯ копия по
  умолчанию (рекурсивно копирует вложенные структуры/Array/Map/String
  тоже — как Python's `deepcopy`, не `copy`), не поверхностная.
  Осталось investigation: где именно живёт struct-присваивание в
  compiler.odin/vm.odin (какой опкод копирует ^Aggregate_Value
  указатель сейчас — `Set_Local`/`Set_Property`), чтобы понять, где
  встраивать вызов `.клонировать()` вместо голого копирования указателя.
- **Хешируемое (Hash)** — тир: тяжёлый, отдельная подсистема. Нужен для
  пользовательских структур как ключей `Соответствие` (сейчас неясно, что
  вообще происходит при попытке — не проверено). Трогает рантайм-хеш
  карт в vm.odin, не typecheck/compile слой, где живёт Стадия 22 — другой
  investigation-скоуп.
- **Итерируемое (Iterable)** — тир: тяжёлый, архитектурный. `для x в`
  сейчас чистый parser-level sugar (Стадия 17, "ноль изменений в
  resolver/type_cheker/compiler/vm" — по факту фиксирован на встроенных
  `Массив`/`Соответствие`). Поддержка пользовательского типа через
  интерфейс потребовала бы переноса части for-in с parser-уровня на
  resolver/typecheck — не расширение существующего sugar-механизма, а
  отдельная фича.

**Порядок, если делать** (подтверждено grilling-сессией, см. TASKS.md):
Печатаемое/Арифметика/По-умолчанию берутся СТРОГО ПОСЛЕДОВАТЕЛЬНО после
Стадии 22 закрытия — НЕ объединяются с ней в один заход (22 уже
de-risked двумя Explore-агентами, лёгкий тир 23 — ещё нет; смешивание
теряет чистый checkpoint). Каждый из трёх получает СВОЙ короткий
Explore/Read-проход перед Порядком работ (не полный двух-агентный, как у
22, но обязательный — 22 показала, что даже "понятный" механизм прятал
реальный блокер, найденный только investigation'ом, не обсуждением).
...
### Стадия 24 (grilled, не Explore-исследовано) — stackful coroutines

**Статус**: архитектурные РЕШЕНИЯ (не investigation-детали) прошли
grilling-сессию — single-thread cooperative, I/O non-goal и синтаксис
ниже переспрошены явно и подтверждены, не просто мимоходом-приняты
из обсуждения. Конкретная архитектура опирается на реальный код
(`execute()`, vm.odin:800), но touch-точки (компилятор/GC/канал-
семантика) НЕ прошли Explore-уровень investigation — Порядок работ ниже
всё ещё архитектурный набросок, не готовый к исполнению план.

**Что**: cooperative stackful-корутины (Go/Lua-style) поверх текущего
байткод-VM. Ключевое наблюдение (см. `lang-design-notes.md` §8) —
дёшево ИМЕННО для этой VM: `VM.frames`/`VM.stack` (vm.odin:18) уже живут
в куче (`[dynamic]`), не на нативном стеке хоста, значит suspend/resume
не требует asm/setjmp/OS-fiber — только смена указателя, какая
`Coroutine{frames, stack}` сейчас "current".

**Архитектура (набросок из обсуждения)**:
```
Coroutine :: struct {
    frames: [dynamic]CallFrame,
    stack:  [dynamic]Value,
    state:  enum { READY, RUNNING, SUSPENDED, BLOCKED_ON_CHAN, DONE },
}
Scheduler :: struct {
    current: ^Coroutine,
    ready:   [dynamic]^Coroutine,  // round-robin очередь
}
```
`execute()` (vm.odin:800, сейчас `for len(vm.frames) > 0 {...}` —
монолитный run-to-completion) переводится на работу над
`vm.current.frames`/`.stack`. Yield — новый выход из цикла (explicit
`дай`/yield-builtin ИЛИ блокирующая channel-операция) просто `return`
из `execute()`: `ip` уже указывает на следующую инструкцию (advance до
return) — resume = повторный вызов `execute(vm)` с тем же `current`,
цикл продолжает без отдельной resume-логики. Внешний scheduler-loop
оборачивает `execute()`: pop из `ready`, прогнать до yield/done,
по `state` — DONE отбрасывается, SUSPENDED возвращается в конец
`ready`, BLOCKED_ON_CHAN уходит в wait-list конкретного канала (не
busy-spin проверка).

**Ключевое scope-решение (grilled, подтверждено явно)**: single-OS-thread
cooperative (Lua/JS-style, БЕЗ реального параллелизма), не M:N с
настоящими OS-потоками (Go-style). Параллелизм осознанно не нужен пока.
Причина: `gc.odin`/весь VM спроектированы под ОДНОГО mutator'а, нулевая
concurrency-safety нигде — M:N потребовал бы concurrent/stop-the-world
GC, отдельный намного больший проект без текущего задела.

**Явный non-goal v1 (grilled, подтверждено явно)**: блокирующий I/O
(`vm_io_native.odin`'s фс/сеть) НЕ кооперируется со scheduler'ом —
блокирует процесс целиком, как сейчас. Мотивация Стадии 24 —
генераторы + CPU-bound конкурентность, не "параллельные HTTP-запросы"
(это отдельно потребовало бы epoll/kqueue-поллер поверх
`vm_io_native.odin`, вне scope). Корутины помогают CPU-bound логике +
explicit-yield-генераторам + channel-координации между собой.

**Синтаксис (grilled, подтверждено)**:
- Запуск корутины — `запусти <выражение-вызов>`, statement-form БЕЗ
  своего блока (в panos блоки закрываются `конец`, не `{}` — этот
  вариант ноль новых grammar-правил не добавляет, синтаксически
  идентичен `возврат <expr>`). Многострочное тело — через уже
  существующий anon-function литерал: `запусти функ() -> Пусто
  ... конец` (тот же синтаксис, что уже работает в
  `кол.отфильтровать(xs, функ(x: Число) -> Булево ... конец)`).
- Тип канала — `Канал(T)`, форма совпадает с уже существующими
  generic-типами (`Опция(T)`, `Массив(T)`).
- Yield — `дай <выражение>`.

**Не исследовано (нужен Explore перед Порядком работ — investigation-
детали, не пользовательские решения)**:
- Компиляция `дай`/`запусти` в опкоды — новые Opcode, compiler.odin.
- Типизация `Канал(T)`-операций (`<-`/`->`), взаимодействие с уже
  существующей generic-инфраструктурой (Стадия 7).
- GC root-walking по МНОЖЕСТВУ стеков корутин (сейчас — один
  `vm.stack`/`vm.frames`, тривиальный корень) — gc.odin.
- Wait-list канала: конкретная структура данных, fairness-гарантии.
- Буферизация канала по умолчанию (0/N) — investigation, не решение.

**Prerequisite**: нет формального (не зависит от Стадий 22/23), но
разумно делать ПОСЛЕ Generics (Стадия 7 ✅, уже готова) — `Канал(T)`
как generic-тип нужен инфраструктуре дженериков.

...
(Сравниваемое/Равнозначное — operator sugar) ✅.
...
требуют Explore. **Приоритет (grilled)**: 23/24/25/26/4/8 — следующие
...
88/88 тестов (было 82, +6 новых: Ord-операторы, Self-тип field-access,
негативный кейс без impl, точное mismatch-сообщение, Equatable-override,
регрессия структурного `==` без impl). `odin build .`/`./lsp`/wasm — все
три цели чисто.

Побочная находка (НЕ баг Стадии 22, предсуществующий): `кол.отсортировать`
(std/коллекции.ps) с T=структура ломает generic-инференс —
`отсортированные[0].x` даёт "попытка получить поле у не-структуры (тип:
?N)" — воспроизведено И БЕЗ единого упоминания Ord/Eq (`a.x < b.x`,
только числа внутри структуры). Значит generic T не резолвится корректно
через `отсортировать`, когда T — структура, независимо от Стадии 22.
Оригинальный Checkpoint-сценарий "сортируется через `<` напрямую" не
демонстрируется через `отсортировать` из-за этого — прямые
`<`/`>`/`<=`/`>=`-тесты подтверждают фичу полностью, сам оператор
работает корректно. Баг не заведён отдельной стадией — не исследован
глубоко, оставлен как известное ограничение для будущего расследования.

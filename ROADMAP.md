...
investigation до плана. **Приоритет (grilled)**: Стадия 22 следующая,
23/24/4/8 — после её закрытия, не параллельно.
...
3. Typechecker: резолв sugar в кейсах `.Less`/`.Greater`/`.LessEqual`/
   ...
   существующий `ctx.call_infos` (0.5 дня).
...
- **Копируемое (Clone)** — тир: средний, но НЕ инфраструктурный — сначала
  дизайн-вопрос, не investigation: что такое "копия" при текущей
  семантике (структуры и так copy-by-value для примитивных полей;
  вопрос — про вложенные ссылочные типы, Array/Map/String внутри
  структуры). Обсудить ПЕРЕД investigation, иначе исследование не на то
  смотрит.

...
**Важно**: НЕ настоящий FFI (см. Стадию 8) — статические Odin-биндинги,
завёрнутые в модуль `графика` тем же способом, что `фс`/`ос`/`ввод_вывод`/
`строки`/`сеть`/`время` (`core/stdlib.odin` + `#+build !js`/`js`-split).

**Расследовано ДО реализации — сняло оба открытых вопроса плана заранее**:
- Не нужен свой `core/raylib_bindings.odin` с ручными `foreign import`.
  Odin's тулчейн уже поставляет `vendor:raylib` — полный готовый биндинг.
  Подтверждено вживую: `import rl "vendor:raylib"` линкуется чисто против
  системного raylib (поставлен через `brew install raylib`, версия 5.5).
  Значит "системный vs vendored raylib" — открытый вопрос плана — снят:
  используется `vendor:raylib` напрямую, никакого своего C-тулчейна.
- `Vector2 :: [2]f32`/`Color :: distinct [4]u8` в vendor:raylib —
  простые массивы, значит на panos-стороне обычные тупы `(Число, Число)`/
  `(Число, Число, Число, Число)`, БЕЗ нового `Type_Kind` (`new_tuple_
  type`, type_cheker.odin:2552, тот же приём, что `получить_сигнал()`'s
  `(Целое, Опция(Строка))`).
- Все нужные для pong функции raylib укладываются в арность ≤3, если
  Vector2/Color передавать ЦЕЛИКОМ (`DrawRectangleV` вместо `DrawRectangle`)
  — существующих `builtin_function_type_1/2/3` достаточно, `_4`/`_5` не
  понадобились.

**Реализовано**: 11 функций (`инициализировать_окно`/`окно_должно_
закрыться`/`закрыть_окно`/`начать_рисование`/`закончить_рисование`/
`очистить_фон`/`нарисовать_прямоугольник`/`нарисовать_круг`/`клавиша_
нажата`/`время_кадра`/`задать_fps`) + 6 нуль-арных геттеров кодов клавиш
(`клавиша_вверх`/`_вниз`/`_влево`/`_вправо`/`_пробел`/`_escape` —
panos не имеет механизма импорта чужих enum'ов как символьных констант,
эти геттеры — обходной путь без нового языкового механизма).
`core/vm_graphics_native.odin` (`#+build !js`, `call_builtin_graphics`,
точный прецедент `call_builtin_io`) + `core/vm_graphics_wasm.odin`
(`#+build js`, паника с понятным сообщением на любой `графика::*` —
точный прецедент `vm_io_wasm.odin`). `demos/pong.ps` — весло игрока
(стрелки вверх/вниз) + простой ИИ-соперник + отскок мяча + счёт в
консоль. Ручной прогон подтверждён: окно открывается, физика/рисование/
клавиатура работают, `Window closed successfully` при штатном выходе.

**Найденное ограничение — актёрная модель и графический цикл.**
`run_scheduler` (vm.odin) переключает процессы ТОЛЬКО когда текущий
`execute()`-вызов возвращается (процесс завершился/упал/приостановился
на ПУСТОМ `получить()`/`получить_сигнал()`) — кооперативность БЕЗ
преемптивности, единственный OS-поток на весь VM (как и раньше — не
регрессия этой стадии, но графика делает эффект видимым впервые). Все
`графика::*` builtin'ы — ОБЫЧНЫЕ синхронные вызовы, как `фс.записать` —
не приостанавливают процесс. Значит `пока графика.окно_должно_закрыться()
== ложь цикл ... конец`, если внутри НИ РАЗУ не встречается `получить()`/
`получить_сигнал()` с пустой очередью, выполняется одним ГИГАНТСКИМ
`execute()`-вызовом от `InitWindow` до закрытия окна — ВСЕ остальные
spawned-процессы (супервизоры, воркеры, что угодно через `запусти`)
голодают и не получают ни единого такта планировщика, пока окно не
закроется. panos не имеет primitive'а "уступить без ожидания сообщения"
(нет scheduler-level timer/yield, см. Стадия 46 investigation) — значит
СЕЙЧАС невозможно совместить графический цикл с ПАРАЛЛЕЛЬНО работающими
акторами в одной программе без явного протокола (сам графический цикл
должен был бы периодически ЖДАТЬ сообщение, а это требует, чтобы кто-то
СНАЧАЛА успел его прислать — курица-яйцо при единственном OS-потоке).
`demos/pong.ps` этой проблемы не показывает — не использует процессы
вообще, `старт()` синхронно ведёт весь игровой цикл. Задокументировано
как честное известное ограничение (`docs/src/language/graphics.md`), не
пофикшено — потребовало бы нового VM-примитива (кооперативный yield без
ожидания сообщения), вне scope этой стадии.

**Файлы**: `core/stdlib.odin` (модуль `графика`, `raylib_vector2_type`/
`raylib_color_type`), `core/vm_graphics_native.odin` (новый),
`core/vm_graphics_wasm.odin` (новый), `core/vm.odin` (диспетчинг в
`call_builtin`), `demos/pong.ps` (новый, + новый каталог `demos/`),
`docs/src/language/graphics.md` (новый) + `SUMMARY.md`.

**Верификация**: `odin build .`/`odin build ./lsp`/`odin build wasm
-target:js_wasm32 -o:size` — чисто (LSP не нуждается в доп. регистрации,
тот же вывод, что в Стадии 46; wasm не тянет `vendor:raylib` благодаря
`#+build !js`). `odin test ./core -define:ODIN_TEST_THREADS=1` — 191/191
без регрессий (графика не покрыта юнит-тестами — открытие окна не
автоматизируется в headless-раннере). Ручной прогон `demos/pong.ps` и
обоих doc-примеров через `/tmp/panos_bin` — окно открывается, рисует,
закрывается штатно (`Window closed successfully`), физика/счёт работают.
...
- Memory ownership аннотации: `(свой)` / `(чужой)`.
...
для `Указатель(T)`/`ff_структура` с `(свой)` не нужно строить
...
- **`Pointer_Value` как новый Value-вариант**: точный прецедент —
  ...
  (exhaustive switch — СЮДА же владение `(свой)`/`(чужой)`, см.
...
**Стадия 47 (первый инкремент, реализовано 18.07.2026) — checkpoint
достигнут, стадия НЕ закрыта целиком**: грилинг-раунд по вопросам 1
(`core:dynlib` + libffi, не одно вместо другого), 2 (libffi — vendored,
статическая линковка ПРЕКОМПИЛИРОВАННОГО архива в `external/libffi/`, по
общей политике проекта "вендорить всё, статика где можно"), 3
(`Число_32` → `Целое(N)`, модификатор НАД существующим `Целое`, не новый
`Type_Kind`) проведён и решения приняты. Реализовано: грамматика
`внешний "libc" функ имя(x: Целое(32)) -> Целое(32)` (`core/token.odin`,
`core/lexer.odin`, `core/parser.odin`), резолв через `dynlib.
load_library`/`symbol_address` с понятными Resolve Error на
несуществующую библиотеку/символ (`core/resolver.odin`), типизация как
обычной функции (`core/type_cheker.odin`), Odin-биндинги libffi
(`core/ffi_bindings.odin`, `#+build !js`), опкод `Call_Foreign` +
компилятор (`core/compiler.odin`), маршаллинг аргументов/возврата в VM
(`core/vm_ffi_native.odin`/`core/vm_ffi_wasm.odin`) — checkpoint
(`getpid()` возвращает реальный PID, проверено сравнением с PID самого
тестового процесса) и `abs(x)` (маршаллинг аргумента) оба работают, e2e
тесты в `core/e2e_test.odin`, документация `docs/src/language/ffi.md`.
**Явно НЕ сделано** (список выше "Что" по-прежнему актуален для
оставшегося объёма): `КСтрока`/строковый marshalling, `Указатель(T)` +
ownership-аннотации, `ff_структура`, callback'и (`ffi_closure`), SIGSEGV
recovery, `Число(32)`/`Число(64)` (float-ширины — сейчас только
`Целое(32)`/`Целое(64)`), платформы кроме `darwin-arm64` (см.
`external/libffi/README.md`). Каждый из этих пунктов — отдельный
следующий инкремент, не автоматически входящий в этот checkpoint.
...
**Инкременты 48-51 (см. отдельные записи ниже, после Стадии 49) закрыли
почти весь оставшийся объём**: замыкания (48, зависимость для callback'ов
позже), `КСтрока`/`Указатель(T)` (49), double-free-риск владения (50),
`Число(N)`/`Целое(8)`/`ff_структура` + удаление `графика` из core (51).
**Оставшееся** из исходного списка "Что" (см. начало секции): callback'и
(`ffi_closure`), SIGSEGV recovery, платформы кроме `darwin-arm64`.
...
**Статус**: закрывает известное ограничение, унаследованное и трижды
продлённое (Стадия 25 → 29 → 31): вложенный `Constructor`/`Struct_
Constructor` под-шаблон ВСЕГДА считался "не покрывает целиком", даже
если сам исчерпывающий (`Событие.Клик(Точка(_, _))` — `Точка(_, _)`
покрывает вообще все точки, но старая проверка это игнорировала,
требуя лишнюю catch-all-ветку). Ложноположительных "исчерпывающе" не
было и раньше (безопасно) — просто раздражало пользователя лишней
веткой.

**Реализация**: `Pattern_Info` получает два родственных, но РАЗНЫХ
булевых поля вместо одного шаблона:
- `fields_fully_covered` — ДАНО, что тег/форма уже зафиксирована этим
  шаблоном (конкретный `tag_index` для `Constructor`, единственная
  форма для `Struct_Constructor`) — покрывают ли ВСЕ под-шаблоны СВОИ
  поля целиком? Это то, что нужно `check_match_coverage` на ВЕРХНЕМ
  уровне ветки — "засчитать `covered[tag_index]`" не зависит от того,
  сколько ВСЕГО вариантов у enum'а (это отдельно трекает
  `covered[]`-массив по каждой ветке).
- `is_exhaustive` — покрывает ли шаблон ВЕСЬ домен СВОЕГО типа целиком
  (не только "свой тег") — для `Constructor` ТОЛЬКО если у enum'а ровно
  один вариант, даже при `fields_fully_covered == true`. Нужен, когда
  шаблон сам используется как под-шаблон РОДИТЕЛЯ на уровень выше.

Оба поля считаются рекурсивно СНИЗУ ВВЕРХ прямо в `classify_pattern` (не
отдельным проходом) — сначала классифицируются под-шаблоны, `fields_
fully_covered` родителя = И по `sub.is_exhaustive` всех под-шаблонов.
`check_match_coverage` читает `pi.fields_fully_covered` вместо старой
поверхностной проверки "любой sub.kind даёт false".

**Найденный по ходу разработки баг (в НОВОМ коде этой стадии, не
регрессия)**: первая версия слила оба понятия в одно поле
`is_exhaustive` с гейтом `variant_count == 1` — это СЛОМАЛО голые (без
скобок) варианты-теги (`Точка`/`Круг`/`Нет`/`Есть` и т.п.) на ВЕРХНЕМ
уровне match'а: `covered[tag_index]` требовал `variant_count == 1`,
хотя top-level ветка должна засчитываться независимо от общего числа
вариантов enum'а (это разные вопросы — "покрывает СВОЙ тег" vs
"покрывает ВЕСЬ enum, если использован как под-шаблон"). Поймано сразу
всем test suite'ом (24 упавших теста — `test_match_missing_variant_*`
и т.п.), разделено на два поля.

**Файлы**: `core/type_cheker.odin` (`Pattern_Info.fields_fully_covered`
+ `.is_exhaustive`, `classify_pattern` — все ветки `Pattern_Ident`/
`Pattern_Constructor`, `check_match_coverage` — `.Constructor`/`.
Struct_Constructor`-кейсы).

**Известный смежный, но НЕ исправленный здесь пробел** (не регрессия,
...
покрывающей ветку того же тега не помечается недостижимой (только
случай "обе ветки claim'ят полное покрытие" ловится как "покрыт
повторно") — полноценный dead-code-анализ для переставленных шаблонов
не входит в эту стадию.

**Верификация**: 2 новых e2e-теста (рекурсивно исчерпывающий вложенный
`Struct_Constructor` без лишней catch-all; негативный — частично
суженный вложенный под-шаблон всё ещё требует catch-all). `docs/src/
language/enums-and-match.md` — новый пример. `odin test ./core` —
145/145 (0 регрессий после исправления бага выше). native/lsp/wasm
сборки чисты.
...
**Статус**: закрывает второй pattern-matching пункт из `language-fails`
("позиционный матчинг разросся по площади поражения") — не полностью
(деструктуризация `пер Тип(a, b) = ...` и обычный `.поле`-доступ
остались позиционными/по имени соответственно, вне scope этой стадии
по запросу — только match-шаблоны в `выбор`), но добавляет
альтернативу позиционной форме там, где она болезненнее всего
(структуры с несколькими полями одного типа, где перестановка молча
меняет смысл).

**Согласовано (grilled)**: синтаксис `Тип(имя: под-шаблон, ...)` (тот
же `:`, что у field-аннотаций в объявлении структуры — не отдельные
фигурные скобки, panos их нигде не использует); частичная форма
разрешена — неупомянутые поля трактуются как неявный `_`, не сужают
exhaustiveness ветки.

**Синтаксис**: `Pattern_Constructor` получает `field_names: [dynamic]
string` (параллельно `args`, пусто целиком у позиционной формы —
нулевой риск регрессии для существующего кода). Парсер решает форму по
ПЕРВОМУ аргументу — `Ident Colon` впереди однозначно сигналит
именованную (ни один валидный позиционный под-шаблон не начинается с
голого `идент:`, неоднозначности в грамматике нет). Новый
`peek_second_token` (core/token.odin) — двухтокенный lookahead без
потребления. Смешивать позиционную и именованную форму в одном
шаблоне — ошибка парсинга.

**Только у структур**: у enum-вариантов нет ИМЁН полей
(`Variant_Decl.types` — список типов, не именованных полей, в отличие
от `Struct_Decl.fields`) — именованная форма для них в принципе
бессмысленна, `classify_pattern` репортит явную diagnostic вместо
попытки угадать.

**Частичность и exhaustiveness**: `classify_pattern` строит `info.
sub_patterns` ФИКСИРОВАННОЙ длины (по числу полей структуры,
позиционно — не по порядку в шаблоне), заполняя упомянутые по имени
позиции классифицированными под-шаблонами, а неупомянутые — неявным
`Pattern_Info{kind = .Wildcard, is_exhaustive = true, ...}`.
`compile_pattern` (compiler.odin) работает НЕИЗМЕНЁННЫМ — видит тот же
позиционный `sub_patterns`-массив, что и раньше, знать не знает про
имена полей вообще.

**Файлы**: `core/parser.odin` (`Pattern_Constructor.field_names`,
разбор именованной формы в `parse_pattern`), `core/token.odin`
(`peek_second_token`), `core/type_cheker.odin` (`classify_pattern`'s
Struct-ветка — именованный путь параллельно позиционному; Enum-ветка —
явный отказ на `field_names`).

**Верификация**: 6 новых e2e-тестов (частичное именованное совпадение;
все поля явно wildcard = catch-all; неизвестное имя поля; смешивание
форм; именованная форма на enum-варианте). `docs/src/language/
enums-and-match.md` — новый раздел с примером. `test.ps` — `классифицировать_точку_именованно`.
`odin test ./core` — 150/150 (0 регрессий). native/lsp/wasm сборки
чисты.
...
**Статус**: по явному запросу — прямое продолжение Стадии 35 (там же
именованные ПОЛЯ в шаблонах), теперь именованные АРГУМЕНТЫ на call
site'ах. Grilled: везде, где есть параметры/поля (функции + конструкторы
структур + методы); всё именовано или всё позиционно, без смешивания и
без частичности (в отличие от шаблонов — вызов не может "не указать"
параметр, default-значений в panos нет).

**Синтаксис — `=`, не `:`**: по прямому указанию — та же нотация, что у
записей `соответствие(ключ = значение)`, не field-аннотаций структур
(которые Стадия 35 переиспользовала для шаблонов). Разбирается ДО
вызова `parse_expr` на значение — `x = 1` как позиционное выражение-
присваивание не теряется по факту: присваивание возвращает `Пусто`, а
`Пусто` нигде не может быть типом параметра, значит старая
интерпретация НИКОГДА не типизировалась бы успешно ни в одной реальной
программе — переиспользование `=` для именованных аргументов не ломает
ни один существующий рабочий код.

**Архитектура**: `Call_Expr.arg_names: [dynamic]string` (параллельно
`args`, пусто целиком = позиционная форма). Новый `resolve_named_call_
args` (type_cheker.odin) — ОДНА общая процедура, переставляет `e.args`
ПРЯМО В AST в порядок объявления параметров ДО любой из ~8 веток
разрешения вызова (обычная функция, 3×метод структуры/enum'а/
интерфейса, конструктор структуры, bounded generic, 2×cross-module
разновидности) — после перестановки все они видят ровно то же самое
`e.args`, что и при позиционном вызове, без единой правки в логике
самих check_expr-циклов.

**Метод — новый reverse-lookup**: `ctx.symbol_to_func_decl` раньше
заполнялась только для топ-уровневых `Function_Decl` (не для методов
внутри `реализация` — методы регистрируются отдельным `case ^Impl_Decl:`
проходом). Добавлена запись туда же для методов — нужна, чтобы достать
исходные ИМЕНА параметров метода (получатель `это` — args[0], исключён
явно, `method_type.params[0]` тот же receiver позиционно).

**Известное ограничение (сознательно, узкое)**: интерфейсные вызовы
(переменная типа интерфейса, динамическая диспетчеризация по имени) не
поддержаны — `obj_type.interface_methods` хранит только резолвленный
`^Type` без исходных имён параметров исходной `Method_Signature`,
дотягиваться до неё отдельным полем не стали ради узкого случая. Явная
diagnostic вместо молчаливого игнорирования.

**Файлы**: `core/parser.odin` (`Call_Expr.arg_names`, разбор в LParen-
кейсе Pratt-цикла), `core/token.odin` (`peek_second_token`, тот же, что
Стадия 35), `core/type_cheker.odin` (`resolve_named_call_args`,
`param_decl_names`/`struct_field_names`/`method_param_names`-хелперы,
`symbol_to_func_decl` для методов, вызовы `resolve_named_call_args` в
8 местах разрешения `Call_Expr`).

**Верификация**: 4 новых e2e-теста (функция+структура+метод в одной
программе с разным порядком по имени; смешивание форм — ошибка;
неизвестное имя — ошибка; повтор имени — ошибка). `docs/src/language/
functions.md` — новый раздел. `test.ps` — `вычесть` с именованным
вызовом. `odin test ./core` — 154/154 (0 регрессий). native/lsp/wasm
сборки чисты.

### Стадия 37 ✅ — Именованная/частичная деструктуризация

**Статус**: по явному запросу ("Реши 2") — закрывает последний
позиционный-only пункт из `language-fails`: конструктор структуры
(Стадия 36) и шаблоны в `выбор` (Стадия 35) уже получили именованные
альтернативы, `пер Тип(a, b) = значение` оставался единственным местом
без неё.

**Синтаксис — `:`, не `=`**: собственный выбор (не переспрашивал
отдельно) — деструктуризация семантически ближе к шаблону матчинга, чем
к вызову: `Точка(x: a)` уже значит "поле x, связать с локальным a" в
контексте `выбор` (Стадия 35), тот же смысл нужен здесь. `пер
Точка(y: b, x: a) = точка` — порядок в списке не обязан совпадать с
порядком объявления полей. Именованная форма может быть ЧАСТИЧНОЙ (`пер
Точка(z: c) = точка` извлекает только `z`) — в отличие от именованных
аргументов вызова (Стадия 36), где частичность запрещена, потому что
вызов не может "не указать" параметр. У тупла именованной формы нет —
только позиционная (нет имён полей, тот же принцип, что enum-варианты
в Стадии 35).

**Архитектура — параллельный массив индексов, не полная перестановка**:
в отличие от `Call_Expr` (Стадия 36, где `e.args` переставляется ЦЕЛИКОМ
в AST под порядок объявления), деструктуризация может быть частичной —
переставлять массив другой длины некуда. Вместо этого новое поле
`Type_Ctx.let_destructure_field_indices: map[Stmt][dynamic]int`,
заполняется в `infer_let_destructure_stmt`: для каждого символа в
`Let_Stmt.names` хранит РЕАЛЬНЫЙ индекс поля в структуре (тот же
порядок/длина, что у `names`) — `[0,1,2,...]` для позиционных форм
(тупл и структура), реальные резолвленные по имени индексы для
именованной. `compiler.odin`'s кодогенерация для деструктуризации
читает `field_indices[i]` вместо сырого `i` как операнд `.Get_Property`
— единственная правка в компиляторе.

**Парсер**: `Let_Stmt.destructure_field_names: [dynamic]string`
(параллельно `names`, пусто целиком = позиционная форма, имеет смысл
только для структурной формы). `parse_destructure_names` получил
параметр `allow_named: bool` — тупл-ветка зовёт с `false` (позиционная
форма — единственная для тупла, `:` там не значит именованную форму, а
просто ведёт к синтаксической ошибке), структурная ветка — с `true`.
Тот же паттерн детекции именованной формы по первому элементу (`Ident`
+ `.Colon`), что у Стадий 35/36, включая diagnostic при смешивании
позиционных/именованных имён в одном списке.

**Типизация**: `infer_let_destructure_stmt` переписан — для именованной
формы валидирует неизвестное имя поля и повтор имени (та же diagnostic-
структура, что Стадии 35/36), для обеих позиционных форм (тупл и
структура) поведение не изменилось (кроме побочного заполнения
`field_indices` тождественной последовательностью).

**Файлы**: `core/parser.odin` (`Let_Stmt.destructure_field_names`,
`parse_destructure_names(p, allow_named)`), `core/type_cheker.odin`
(`Type_Ctx.let_destructure_field_indices`, переписанный
`infer_let_destructure_stmt`), `core/compiler.odin` (кодогенерация
деструктуризации читает `field_indices[i]`).

**Верификация**: 6 новых e2e-тестов (реордер полного набора полей;
частичное извлечение; неизвестное имя поля — ошибка; повтор имени поля
— ошибка; смешивание форм — ошибка; тупл отклоняет именованный
синтаксис). `docs/src/language/structs-and-methods.md` — новый раздел
"Именованная деструктуризация". `test.ps` — `Игрок(здоровье: ...,
имя: ...)` и частичное `Игрок(координаты: ...)` внутри
`проверка_деструктуризации`. `odin test ./core` — 160/160 (0
регрессий). native/lsp/wasm сборки чисты.

### Стадия 38 ✅ — Monitor-примитив для actor model (`наблюдать`/`получить_сигнал`)

**Статус**: по запросу ("Давай с supervisor tree/monitor") — закрывает
пункт из `language-fails` "актёрная модель пока никого не умеет
супервайзить" (Стадия 24 дала message-passing, но без единого способа
узнать, что процесс упал). Grilled: только сам примитив в этой стадии,
не полноценный supervisor tree со стратегиями рестарта — тот станет
обычным panos-кодом ПОВЕРХ этого примитива позже (Erlang-модель:
supervisor — не более чем monitor + restart-логика в стд-либе).

**Изоляция крашей — ключевая находка planning'а.** Monitor, видящий
только штатное завершение, а не краши, был бы гораздо слабее Erlang'а —
а ЛЮБОЙ runtime-краш (`паника()`, деление на ноль, индекс за границей
массива, ключ не найден в `соответствие`) до этой стадии был
`fmt.panicf`, роняющим ВСЮ программу (Odin panic невосстановим), а не
только упавший процесс. Раскопано: `.значение()`/`.причина()`/
`.ожидать()`/`.ожидать_ошибку()` у Опции/Результата — НЕ нативный код
`vm.odin`, а обычные panos-функции из `PRELUDE_SOURCE`
(`core/prelude.odin:98-214`), зовущие builtin `паника(...)` — единая
реальная точка user-triggered краша, а не 49 разрозненных `fmt.panicf`
по файлу. Итоговый catchable-набор — `паника()`, `.Int_Divide`/
`.Modulo` при делителе 0, `.Get_Index`/`.Set_Index` (индекс/ключ) — всё
остальное (`.Call` на не-функцию и т.п.) осталось фатальным `fmt.panicf`
как нарушение инварианта, недостижимое из валидной по типам программы.

**Механизм — новый `Exec_Result.Crashed`**, тот же паттерн, что уже
даёт `.Suspended` у `.Receive`: catchable-сайты выставляют `VM.crash_
message` и возвращают `.Crashed` вместо `fmt.panicf`. `run_scheduler`
решает дальнейшую судьбу: для "старт()" (i==0) — по-прежнему
`fmt.panicf("%s", vm.crash_message)`, тот же текст, что раньше — три
существующих теста на `паника()` не регрессируют. Для остальных
процессов — изоляция: `is_alive = false`, рассылка сигнала
наблюдателям, удаление из `vm.processes`, ВСЯ остальная программа
продолжает работать.

**Доставка — отдельный канал, не типизированный mailbox.** `Процесс(T)`
ждёт сообщения только типа T — DOWN-уведомление о чужом падении другого
типа туда не влезает. Новый `Process_Value.signals: [dynamic]Value`
(параллельно `mailbox`, тот же FIFO) + builtin `получить_сигнал()` —
тот же suspend/resume паттерн, что `получить()` (свой опкод `.Receive_
Signal`), но с ФИКСИРОВАННЫМ типом `(Целое, Опция(Строка))` — не нужен
per-функция InferVar, т.к. сигналы приходят от процессов с разным T,
общее только id и причина.

**`наблюдать(процесс)`** — обычный `.Call_Builtin` (не suspend/resume,
регистрация всегда синхронна и мгновенна), работает для `Процесс(T)` с
любым T (типизация не знает и не должна знать T цели). Наблюдение за
уже мёртвым процессом даёт немедленный синтетический сигнал `Есть("процесс
уже не существует")` — иначе `получить_сигнал()` вызывающего зависал бы
навечно (симметрично тому, что `отправить()` на мёртвый процесс —
тихий no-op, здесь вместо тишины сигнал).

**Именование — свой выбор**: `наблюдать`/`получить_сигнал` — глаголы,
как `получить`/`отправить`/`паника`, не существительное "монитор".
Плюс новый метод `.номер()` на `Процесс(T)` (единственный пока что) —
даёт `Целое` id для сравнения с сигналами (`value_equals`, vm.odin, не
имеет ветки для `^Process_Value` — сравнение хэндлов через `==` не
работает даже сегодня, id-сравнение проще, чем чинить попутно).

**Найденный по ходу баг планировщика**: deadlock-guard в `run_scheduler`
проверял ТОЛЬКО пустой `mailbox`, чтобы решить, пропускать ли процесс —
процесс, приостановленный на `получить_сигнал()` с пустыми signals,
ложно считался "нечего делать" и никогда не получал второй шанс
исполниться после прихода сигнала (живой дедлок, пойман вручную при
первом smoke-тесте). Исправлено — гейт теперь `len(mailbox) == 0 &&
len(signals) == 0`.

**Файлы**: `core/vm.odin` (`Exec_Result.Crashed` + `VM.crash_message`,
4 catchable-сайта, `run_scheduler` (обе ветки уведомляют наблюдателей),
`notify_watchers`, `call_builtin` — `наблюдать`, `execute()` —
`.Receive_Signal`, `invoke_collection_method` — `.номер()`,
deadlock-guard фикс), `core/compiler.odin` (`Process_Value.watchers`/
`.signals`, `Opcode.Receive_Signal`, спец-ветка компиляции `получить_
сигнал`), `core/type_cheker.odin` (`BUILTIN_CTORS`: `наблюдать`,
`получить_сигнал`; новая ветка `obj_type.kind == .Process` для
`.номер()`), `core/resolver.odin` (оба имени в `install_standard_
symbols`), `core/gc.odin` (разметка `signals` в `mark_value`/
`mark_roots`).

**Верификация**: 6 новых e2e-тестов (штатное завершение даёт (id, Нет);
краш даёт (id, Есть(причина)) БЕЗ падения всей программы; деление на
ноль в дочернем процессе изолируется; индекс за границей массива в
дочернем процессе изолируется; наблюдение за уже мёртвым процессом даёт
немедленный сигнал; несколько наблюдателей на одном ребёнке — все
уведомлены). `docs/src/language/processes.md` — новый раздел
"Наблюдение за процессом". `test.ps` — `проверка_наблюдения()`.
`odin test ./core` — 166/166 (0 регрессий, включая 3 существующих
теста на `паника()` из "старт()" — фатальность корневого краша не
изменилась). native/lsp/wasm сборки чисты.

### Стадия 39 ✅ — Единая hard-reserved политика зарезервированных слов

**Статус**: по запросу ("приведи к единой политике", затем "так может
сделать это keywords?", затем явно подтверждено "всё целиком" — grilled,
с явным предупреждением о breaking change) — раньше было три ad-hoc
политики без единого принципа (`language-fails`, уже отражено в этой
сессии): жёсткие keyword'ы лексера (`выбор`/`тип`/`функ` и т.п.) —
никогда не затеняемы; `в` (for-in) — вообще не резервировано, сравнение
по тексту токена в одной точке грамматики; builtin-функции (`получить`/
`отправить`/`себя`/`паника`/`наблюдать`/`получить_сигнал`/`длина`/
`Ошибка`) — свободно затеняемые `.Builtin`-символы в резолвере (Стадия
24 явно ВВЕЛА эту затеняемость из-за коллизии с существующими тестами).
Первая попытка (grilled) — задокументировать существующее поведение как
"два тира, оба легитимны" (мягкий/жёсткий). Пользователь пересмотрел
после уточняющего вопроса про `в`: обе категории должны стать
hard-reserved, без исключений — breaking change принят осознанно.

**Механизм — два тира по-прежнему, но оба unshadowable:**
- `в` переведена из text-compare (`in_tok.data == "в"`) в настоящий
  `Token_Kind.In` лексера (token.odin/lexer.odin), как `как`/`запусти`.
  `parse_for_stmt_into` (parser.odin) теперь проверяет `in_tok.kind !=
  .In` вместо сравнения текста — попытка использовать `в` как
  идентификатор даёт ту же "ожидается идентификатор", что `тип`/`пер`.
- 8 builtin-имён остаются `.Ident`-токенами лексера (перевод в
  настоящие Token_Kind потребовал бы новых grammar-продукций на каждое
  — намного больше риска ради идентичного наблюдаемого результата), но
  резолвер теперь ВСЕГДА отклоняет коллизию с ними, а не только
  top-level: `register_named_symbol` (module-scope, функции/типы)
  лишилась исключения `kind != .Builtin` — теперь коллизия с builtin'ом
  и коллизия с обычным user-decl репортятся одинаково. Новый
  `check_not_reserved` (resolver.odin) — вызывается на КАЖДОЙ
  регистрации ЛОКАЛЬНОГО `.Variable`-символа (параметры функций/лямбд,
  `пер`/`конст`, деструктуризация, for-in имена, pattern-биндеры в
  `выбор`) — эти места НЕ шли через `register_named_symbol` и по
  умолчанию разрешают затенение внешних имён (обычная вложенная область
  видимости), так что без явной проверки reserved-имя оставалось бы
  локально затеняемым. `RESERVED_BUILTIN_NAMES` — общий package-level
  список, используется и `install_standard_symbols` (регистрация), и
  `check_not_reserved` (проверка) — не расходится.

**Обратная совместимость сломана осознанно** — найдено и исправлено по
ходу верификации (полный `odin test ./core` после реализации, не
предугадано заранее):
- `core/e2e_test.odin`: `пер в = ...` (5 мест), `Прямоугольник(ш, в) ->
  ш * в` (pattern-биндер, 2 места), `пер длина: функ(...) = ...` (1
  место), `функ получить() -> ...` (2 отдельных top-level функции в
  table-driven тестах) — все переименованы на нейтральные имена, ни
  один тест не тестирует именно эти имена по существу.
- `std/сеть/http.ps`: **публичная** `экспорт функ получить(ссылка) ->
  ...` переименована в `запросить` — единственное изменение с реальным
  API-эффектом (`получить_с_заголовками` НЕ переименована — другой
  идентификатор, коллизии нет). Обновлены все зависимые места:
  `test_http.ps`, `docs/src/language/networking.md` (пример +
  сигнатура), `docs/src/language/modules.md` (упоминание в прозе).
- `test.ps`/`docs/src/language/enums-and-match.md`: тот же `Прямоугольник(ш,
  в) -> ш * в` pattern-биндер, что в e2e-тестах — тоже переименован.

**Файлы**: `core/token.odin` (`Token_Kind.In`), `core/lexer.odin`
(`case "в": return .In`), `core/parser.odin` (`in_tok.kind != .In`),
`core/resolver.odin` (`RESERVED_BUILTIN_NAMES`, `check_not_reserved`,
`register_named_symbol` без исключения для `.Builtin`, вызовы `check_
not_reserved` в 6 местах регистрации локальных `.Variable`-символов),
`core/e2e_test.odin` (10 переименований), `std/сеть/http.ps` +
3 зависимых файла, `docs/src/language/basic-types.md` (переписан раздел
"Зарезервированные слова" — единая политика вместо "мягкий/жёсткий тир").

**Верификация**: `odin test ./core` — 166/166 (0 регрессий после
исправления обнаруженных коллизий). Ручные smoke-тесты: `пер в = 5`/
`функ получить()`/`пер получить = 5` (top-level и локально) — все дают
"зарезервированное имя"/"ожидается идентификатор"; `для x в коллекция`
по-прежнему работает. `test.ps`/`test_toml.ps`/`test_http.ps` — sanity.
native/lsp/wasm сборки чисты.

### Стадия 40 ✅ — Супервизия: `реализация Модуль.Интерфейс для Тип` + `std/супервизор.ps`

**Статус**: по запросу ("приступа к супервизии", после явного
grilled-подтверждения "чистая std-библиотека" — без новых VM-примитивов)
— закрывает второй пункт из `language-fails` после Стадии 38: monitor
есть, самой супервизии не было.

**Гетерогенность потребовала ре-grilling и найденного языкового
блокера.** Первое предложение — однотипный супервизор (`Массив`
однотипен в panos) — пользователь отклонил ("нужно больше", полноценное
дерево разнотипных детей, как в Erlang). Дизайн через интерфейс
(`Массив(ДочерняяЗадача)`, каждый тип задачи реализует общий интерфейс)
технически даёт гетерогенность, НО эмпирически проверено: `реализация
Модуль.Интерфейс для Тип` **не парсится вообще** — `parse_impl_decl`
(parser.odin) читал имя интерфейса как ОДИН `Ident`, без `.`-квалификации
— пользователь ИЗ СВОЕГО файла не мог реализовать интерфейс, объявленный
в `std/супервизор.ps`. Второй раунд grilling подтвердил: нужен маленький
языковой фикс (не supervisor-специфичный — общеполезен для любого
cross-module интерфейса), не откат к однотипному дизайну.

**Языковой фикс**: `Impl_Decl.interface_module` (новое поле,
parser.odin) — `parse_impl_decl` читает опциональную `.`-квалификацию
ПЕРЕД проверкой на `для`; если квалификация есть, а `для` нет
(`реализация Модуль.Тип` без интерфейса — расширение методом чужого
типа, другая фича) — понятная синтаксическая ошибка, не крэш. Резолв
(type_cheker.odin, единственная точка, где резолвится `d.interface_
name`, общая для Struct/Enum impl'ов) переиспользует ТОТ ЖЕ путь, что
уже применяет `case ^Type_Qualified:` для `модуль.Тип` в аннотациях
типов — module lookup → `.Module`-kind check → `imported_module.
exports` — ни одной новой diagnostic-инфраструктуры.

**`std/супервизор.ps`** — one_for_one супервизия поверх `наблюдать`/
`получить_сигнал` (Стадия 38), чистый panos-код:
- `ДочерняяЗадача` — экспортированный интерфейс (`запустить() ->
  Целое`, `имя() -> Строка`), реализуется КАЖДЫМ типом задачи в СВОЁМ
  файле через новый языковой фикс — гетерогенность через динамическую
  диспетчеризацию интерфейса, не через generic-параметр массива.
- **Фабрика без замыканий**: у panos-лямбд нет захвата внешних локалей
  (проверено — ссылка на локал внешней функции даёт `Compiler Error`
  компилятора). `запустить()` поэтому содержит буквальный `запусти
  <функция>(литеральные, аргументы)` в своём теле — конфигурация
  "зашита" при объявлении, не передаётся заново при рестарте (тот же
  принцип, что Erlang child spec `{Module, Function, Args}`). Метод сам
  же вызывает `наблюдать(proc)` и возвращает `.номер()`.
- **Restart tracking по ПОЗИЦИИ, не по id** — id меняется на каждый
  рестарт; `id_ов`/`счётчики` — параллельные массивы той же
  длины/порядка, что `задачи`, мутируются на месте (массивы — reference-
  тип, мутация видна вызывающему).
- **Transient-семантика**: штатное завершение (`Нет`) не рестартует.
  Краш (`Есть(причина)`) рестартует до `макс_рестартов_на_ребёнка`, затем
  сам супервизор `паника()`-ит с сообщением, называющим ребёнка (`.
  имя()`) и причину — "let it crash", тот же принцип, что у остального
  языка. Лимит — простой счётчик за всё время жизни супервизора, БЕЗ
  временного окна (в panos нет time-модуля, значит нет способа
  реализовать Erlang-стиль "N рестартов за M секунд").

**Побочно найдено, НЕ являющееся частью этой задачи**: pre-existing
parser-квирк — литерал с унарным минусом (`-1`) на новой строке СРАЗУ
после закрывающего `конец` блока читается как ПРОДОЛЖЕНИЕ предыдущего
выражения (бинарное вычитание), а не новый unary-minus statement — тот
же класс ambiguity, что уже отмечен про `'(' в начале statement'а`.
Обходится через `возврат -1` (keyword не может быть продолжением
выражения) — используется в `std/супервизор.ps`, сама ambiguity не
чинится (отдельный вопрос, вне scope).

**Файлы**: `core/parser.odin` (`Impl_Decl.interface_module`,
`parse_impl_decl`), `core/type_cheker.odin` (резолв `iface_sym` с веткой
на `interface_module`), `std/супервизор.ps` (новый),
`docs/src/language/interfaces.md` (раздел "Интерфейс из другого
модуля"), `docs/src/language/processes.md` (раздел "Супервизия"),
`test_supervisor.ps` (новый, репо-корень, требует `импорт`),
`fixtures/supervisor_fixture_main.ps` (новый).

**Верификация**: 4 новых e2e-теста (позитив — cross-module impl +
полный supervisor-цикл через `run_module_file`, рестарт до лимита, затем
фатальная `паника()` с ожидаемым сообщением через `testing.expect_
assert`; три негатива — неизвестный модуль, модуль не экспортирует имя,
квалификация без `для`). `odin test ./core` — 170/170 (0 регрессий).
Ручной прогон `test_supervisor.ps` и обоих новых doc-примеров через
`/tmp/panos_bin` — подтверждено вручную. native/lsp/wasm сборки чисты.

### Стадия 41 ✅ — Супервизия: стратегии `ВсеЗаОдного`/`ОстальныеЗаОдним`

**Статус**: по запросу ("продолжи с супервизией") — grilled: из четырёх
оставшихся гапов после Стадии 40 (one_for_all/rest_for_one, link-
примитив, динамические дети, временное окно лимита) выбраны restart-
стратегии — расширяет уже готовый `std/супервизор.ps`, чисто
библиотечная доработка, без языковых/VM изменений.

**Найденное ограничение, честно задокументированное (grilled отдельно,
подтверждено)**: в panos нет примитива для принудительной остановки
процесса извне (только `наблюдать`/`отправить`/`получить`). Erlang при
`one_for_all`/`rest_for_one` СНАЧАЛА завершает выживших siblings, ПОТОМ
рестартует всех — здесь это невозможно: выжившие процессы просто
продолжают жить осиротевшими (та же природа, что у процессов,
осиротевших после возврата `старт()`, Стадия 24), пока рядом спавнятся
свежие. Не буквальный "рестарт группы", а "спавн свежей группы, старая
тихо простаивает" — альтернативой было бы сначала добавить kill-
примитив (отдельная, более крупная VM-задача), решено делать честно и
задокументировать, не блокировать этим саму фичу.

**`Стратегия`** — новый экспортированный `перечисление` (`ОдинЗаОдного`/
`ВсеЗаОдного`/`ОстальныеЗаОдним`, Erlang-терминология one_for_one/
one_for_all/rest_for_one), новый параметр `супервизировать(задачи,
стратегия, макс_рестартов)` — **breaking change** сигнатуры (все
существующие вызовы обновлены: `test_supervisor.ps`, `fixtures/
supervisor_fixture_main.ps`, оба doc-примера в `processes.md`).

**Двойной смысл `макс_рестартов` в зависимости от стратегии** (осознанно,
задокументировано, не отдельные параметры — простота важнее): у
`ОдинЗаОдного` — per-child cap, свой счётчик у каждого ребёнка (`счётчики`,
без изменений от Стадии 40). У `ВсеЗаОдного`/`ОстальныеЗаОдним` — счётчик
ОБЩИЙ на весь супервизор (`групповой_счётчик`, новая локальная переменная
в `супервизировать`) — единица подсчёта "сколько раз рестартовала вся
группа целиком", не "сколько раз рестартовал конкретный ребёнок".

**Механика**: `обработать_краш` теперь возвращает `Целое` (новое
значение `групповой_счётчик` — неизменённое для `ОдинЗаОдного`, у
которого своя per-child бухгалтерия) — `супервизировать`'s цикл
переприсваивает `групповой_счётчик = выбор причина { Нет ->
групповой_счётчик; Есть(текст) -> обработать_краш(...) } конец`.
`Целое`-локали НЕ reference-тип (в отличие от `Массив`) — мутация внутри
функции не была бы видна вызывающему, отсюда return-and-reassign вместо
mutate-in-place, которым пользуются `id_ов`/`счётчики`. Для группового
рестарта — единый цикл `от..задачи.длина()`, `от = 0` для `ВсеЗаОдного`,
`от = найденный` (индекс упавшего) для `ОстальныеЗаОдним` — единственная
разница между двумя стратегиями, весь остальной код общий.

**Файлы**: `std/супервизор.ps` (`Стратегия`, новая сигнатура
`супервизировать`, переписанный `обработать_краш`), `test_supervisor.ps`
+ `fixtures/supervisor_fixture_main.ps` (обновлённые вызовы),
`fixtures/supervisor_one_for_all_fixture_main.ps` + `fixtures/
supervisor_rest_for_one_fixture_main.ps` (новые), `docs/src/language/
processes.md` (раздел "Супервизия" переписан под три стратегии +
ограничение).

**Верификация**: 2 новых e2e-теста (`ВсеЗаОдного`/`ОстальныеЗаОдним`,
через `run_module_file` + `testing.expect_assert` на итоговое сообщение
после исчерпания группового лимита), 1 существующий обновлён (новый
формат сообщения `супервизор (один-за-одного): ...`). `odin test ./core`
— 172/172 (0 регрессий). Ручные smoke-тесты обеих новых стратегий и
обновлённого doc-примера через `/tmp/panos_bin` — подтверждено.
native/lsp/wasm сборки чисты.

### Стадия 42 ✅ — Kill-примитив (`убить`)

**Статус**: по запросу ("продолжай реализацию" → grilled: kill/link/
динамические дети — выбран kill) — закрывает честно задокументированное
ограничение Стадии 41 (групповые restart-стратегии не могли по-
настоящему остановить выживших siblings, только осиротить их). VM-
примитив, в отличие от Стадий 40/41 (чистая std-библиотека).

**`убить(процесс: Процесс(T)) -> Пусто`** — типобезопасен, зеркалит
`наблюдать` по форме (любой T, обычный `.Call_Builtin`, не suspend/
resume). Переиспользует ТУ ЖЕ очистку, что `run_scheduler` уже делает
для `.Completed`/`.Crashed` (Стадия 38/41) — `is_alive = false`,
`notify_watchers` с причиной `"процесс принудительно остановлен
(убить())"`, очистка `frames`/`stack`/`mailbox`/`watchers`/`signals`,
`unordered_remove` из `vm.processes` — выполняется СИНХРОННО прямо
внутри `call_builtin`, а не через цикл планировщика: с точки зрения
`получить_сигнал()` наблюдателя убийство неотличимо от обычного краша.

**Безопасность мид-scheduler-loop мутации проверена явным разбором**:
`убить()` может удалить целевой процесс из `vm.processes` ПОКА
`run_scheduler`'s внешний цикл "приостановлен" на вызове `execute(vm)`
для СОВСЕМ ДРУГОГО индекса (вызывающего процесса). Разобрано: `process
:= vm.processes[i]` в `run_scheduler` — копия УКАЗАТЕЛЯ, не индекса;
последующая запись `process.frames = vm.frames` пишет в ТОТ ЖЕ объект
независимо от того, что случилось с МАССИВОМ `vm.processes` за это
время (`unordered_remove` в другом месте меняет порядок элементов
МАССИВА, но не адрес уже полученного объекта). Худший случай —
незначительная фейрность round-robin в течение ОДНОГО раунда
(процесс может быть пропущен или посещён дважды подряд), не
краш/повреждение — тот же класс тradeoff, что уже принят у
существующего `unordered_remove` в `.Completed`/`.Crashed`-ветках.

**Два явных запрета** (рантайм-проверки, не тайпчекер — типизация не
знает "текущий процесс" статически):
- **Самоубийство** (`убить(себя())`) — `execute()` не умеет прервать
  сам себя посреди своих инструкций; `убить()` — обычный синхронный
  builtin-вызов, простой `return` из него не остановил бы оставшееся
  тело процесса. Фатальная ошибка вместо неопределённого поведения.
- **Убийство "старт()"** (процесс #0, `target.id == 0`) — корневой
  процесс уже имеет особую семантику (крах/завершение останавливает
  всю программу); убийство его извне потребовало бы прокидывать сигнал
  наверх в `run_scheduler` из глубины `call_builtin` — не стоит
  сложности ради узкого кейса ("за старт() и так некому наблюдать").

**Осознанно НЕ сделано в этой стадии** (grilled — сузили scope,
отдельный follow-up): `std/супервизор.ps` пока НЕ переписан на
использование `убить()` для по-настоящему честных `ВсеЗаОдного`/
`ОстальныеЗаОдним` — супервизор трекает только `Целое`-id детей (не
типобезопасные `Процесс(T)`-хэндлы, генерик по T не может быть
однородным в гетерогенном супервизоре), а `убить()` типобезопасен и
требует хэндл. Wiring этого потребовал бы либо третьего метода в
интерфейсе `ДочерняяЗадача` (снова breaking change контракта), либо
отдельного id-based kill-примитива — оставлено как явный будущий шаг,
не проще совместить с этой стадией без размытия её scope.

**Файлы**: `core/vm.odin` (`case "убить"` в `call_builtin`),
`core/type_cheker.odin` (`BUILTIN_CTORS`: `убить`), `core/resolver.odin`
(`RESERVED_BUILTIN_NAMES`), `docs/src/language/processes.md` (раздел
"Принудительная остановка").

**Верификация**: 4 новых e2e-теста (убийство работающего процесса +
уведомление наблюдателя + изоляция; убийство уже мёртвого — no-op;
самоубийство — фатальная ошибка; убийство "старт()" — фатальная
ошибка). `odin test ./core` — 176/176 (0 регрессий). Ручные smoke-тесты
всех четырёх сценариев через `/tmp/panos_bin` — подтверждено.
native/lsp/wasm сборки чисты.

### Стадия 43 ✅ — `std/супервизор.ps` подключён к `убить()`

**Статус**: по запросу ("выполни следующий шаг") — закрывает явно
отложенный в Стадии 42 follow-up: групповые restart-стратегии
(`ВсеЗаОдного`/`ОстальныеЗаОдним`, Стадия 41) теперь по-настоящему
останавливают выживших siblings перед рестартом, вместо того чтобы их
осиротить.

**Третий метод интерфейса, а не отдельный id-based kill.** Из двух
вариантов, обозначенных в Стадии 42 (третий метод `ДочерняяЗадача` vs
отдельный `убить_по_id(Целое)`), выбран первый — сохраняет ОДИН kill-
механизм в языке (типобезопасный `убить(Процесс(T))`), не плодит два
разных способа остановить процесс. Цена — очередной breaking change
контракта `ДочерняяЗадача` (третий раз за три стадии: Стадия 40 задала
интерфейс, Стадия 41 не трогала его, эта стадия расширяет).

**`остановить() -> Пусто`** — новый метод интерфейса. Каждая конкретная
задача ДОЛЖНА запоминать свой последний `Процесс(T)`-хэндл (в
`Опция(Процесс(T))`-поле своей структуры — `Опция`, потому что до
первого `запустить()` хэндла ещё нет), т.к. `убить()` типобезопасен и
принимает хэндл, а супервизор сам хранит только `Целое`-id (T разный у
разных задач в гетерогенном супервизоре, единообразно хранить хэндлы
негде). `запустить()` пишет `это.последний = Опция.Есть(proc)` —
структуры в panos reference-типы, мутация поля через `это` видна на
следующем вызове через ТОТ ЖЕ интерфейсный хэндл (подтверждено
эмпирически перед реализацией). `остановить()` читает поле, зовёт
`убить()`, если хэндл есть — `если это.последний.есть() тогда убить(это.
последний.значение()) конец`, без `иначе` (Пусто-функция, guard-clause
стиль, уже установленный в `std/сеть/http.ps`).

**Групповой рестарт теперь**: `задачи[i].остановить(); id_ов[i] =
задачи[i].запустить()` для каждого `i` в диапазоне — вызов `остановить()`
на УЖЕ упавшем ребёнке (`i == найденный`) безвреден (`убить()` на мёртвой
цели — тихий no-op, само собой уже гарантировано Стадией 42), так что
никакого условного `если i != найденный` не нужно — единый цикл для всех.

**Файлы**: `std/супервизор.ps` (`ДочерняяЗадача.остановить()`, вызов
перед повторным `запустить()` в `обработать_краш`), `test_supervisor.ps`
+ 3 fixture-файла (`supervisor_fixture_main.ps`, `supervisor_one_for_
all_fixture_main.ps`, `supervisor_rest_for_one_fixture_main.ps`) —
task-структуры получили `последний: Опция(Процесс(T))`-поле и
`остановить()`, `docs/src/language/processes.md` (пример + описание
`запустить()`/`остановить()` переписаны, параграф про "ограничение"
заменён на описание настоящего группового рестарта).

**Верификация**: e2e-тесты (Стадии 40-41) не изменились СОДЕРЖАТЕЛЬНО
(ожидаемые panic-сообщения те же, только внутренняя механика группового
рестарта поменялась) — все проходят без изменений. `odin test ./core`
— 176/176 (0 регрессий). Ручные smoke-тесты всех трёх fixture-сценариев
и doc-примера через `/tmp/panos_bin` — подтверждено. native/lsp/wasm
сборки чисты.

### Стадия 44 ✅ — Link-примитив (`связать`)

**Статус**: по запросу ("давай link-примитив") — закрывает последний из
трёх крупных пунктов, оставшихся в `language-fails` после Стадии 43
(link/временное окно лимита/динамические дети; временное окно осталось
отдельным пунктом — требует целого time-модуля с нуля, не сделано).

**`связать(процесс: Процесс(T)) -> Пусто`** — двусторонняя связь: крах
ЛЮБОЙ стороны (не штатное завершение — тот же принцип, что у Erlang:
`normal` exit не убивает linked процессы) каскадно завершает ДРУГУЮ.
Типобезопасен, зеркалит `наблюдать`/`убить` по форме.

**Panos сознательно НЕ реализует Erlang-style `trap_exit`.** В Erlang
один примитив (`link`) покрывает и "уведоми" и "убей вместе со мной"
через переключатель `trap_exit`; в panos эти два поведения — уже ДВА
РАЗНЫХ примитива (`наблюдать` = уведоми, `связать` = уведоми И убей),
появившихся в РАЗНЫХ стадиях (38 и 44) по независимым причинам — раз
оба поведения уже доступны раздельно, третьего переключателя не нужно.

**`terminate_process` — вынесенная общая функция**, устраняет
дублирование, которое уже завелось между `run_scheduler`'s `.Completed`/
`.Crashed`-веткам и `call_builtin`'s `"убить"` (Стадия 42): единая точка
"процесс окончательно мёртв" — `is_alive=false`, `notify_watchers`,
КАСКАДНОЕ завершение `target.links` (новое, только при ненормальном
`reason`), очистка полей, `unordered_remove` из `vm.processes`. Cycle
guard (`if !target.is_alive do return`) в начале — не только защита от
двойной обработки, но и КОРРЕКТНОСТЬ каскада: A↔B двусторонняя связь
означает, что завершение A (устанавливает `A.is_alive=false` ДО начала
цикла по `A.links`) рекурсивно вызовет `terminate_process(B)`, чьё
СОБСТВЕННОЕ каскадирование через `B.links` попыталось бы вернуться к A
— guard делает этот обратный виток немедленным no-op.

**Root ("старт()", процесс #0) запрещён и как цель, И как инициатор**
связи — асимметрия с `убить()` (тот запрещает root только как цель):
если бы САМ root мог вызвать `связать(other)`, root оказался бы записан
в `other.links`, и БУДУЩИЙ каскад (если `other` когда-нибудь упадёт)
попытался бы завершить root через `terminate_process` — а у той нет
способа воспроизвести особую семантику root'а (программа заканчивается
целиком), это умеет только `run_scheduler`'s собственная ветка `i == 0`.
Оба направления блокируются явной фатальной ошибкой на этапе `связать()`,
не отложенным крахом при каскаде.

**Самолинковка разрешена** (в отличие от `убить(себя())`) — безвредна,
тот же cycle guard её обезвреживает, а не запрещает искусственно.

**Побочно найдено, НЕ являющееся частью этой задачи и НЕ багом связать()**:
при повторных прогонах `odin test ./core` с параллельным (по умолчанию,
10 потоков) тестраннером изредка (не при каждом прогоне) флакует ДВА
СУЩЕСТВОВАВШИХ ДО этой стадии теста — `test_printable_interface_used_
by_print`/`test_non_printable_struct_falls_back_to_structural_dump` —
оба используют `run_code_capture_stdout` (общий, ГЛОБАЛЬНЫЙ `os.stdout`
на время подмены), не потокобезопасно под параллельным ранером —
провалившийся прогон буквально содержит СМЕШАННЫЙ вывод из ДРУГОГО
теста ("panos\nТочка(3, 4)" — "panos" из чужого прогона). Подтверждено:
`-define:ODIN_TEST_THREADS=1` — 182/182 стабильно чисто на трёх подряд
прогонах; те же тесты изолированно (`ODIN_TEST_NAMES` на все новые
`связать()`-тесты) — 5/5 прогонов чисты под параллельным ранером тоже.
Не связано с actor-model кодом вообще (тесты о `Печатаемое`), не чиню —
отдельная, узкая проблема потокобезопасности test-хелпера, вне scope.

**Файлы**: `core/compiler.odin` (`Process_Value.links`), `core/vm.odin`
(`terminate_process`, `case "связать"`, рефакторинг `run_scheduler`'s
`.Completed`/`.Crashed` и `case "убить"` на переиспользование),
`core/type_cheker.odin` (`BUILTIN_CTORS`: `связать`), `core/resolver.odin`
(`RESERVED_BUILTIN_NAMES`), `docs/src/language/processes.md` (раздел
"Двусторонняя связь").

**Верификация**: 6 новых e2e-тестов (каскад краша через связь;
штатное завершение НЕ каскадирует; самолинковка безвредна; мёртвая
цель — no-op; связать() из root — фатально; связать() на root —
фатально). `odin test ./core -define:ODIN_TEST_THREADS=1` — 182/182 (0
регрессий), см. заметку про флаки выше. Ручные smoke-тесты всех шести
сценариев через `/tmp/panos_bin` — подтверждено. native/lsp/wasm сборки
чисты.

### Стадия 45 ✅ — Динамические дети супервизора (qualified `запусти` + per-child relay)

**Статус**: по запросу ("продолжай супервизию" → "динамические дети") —
закрывает предпоследний пункт `language-fails` (остаётся только
временное окно лимита рестартов, требует time-модуля с нуля).
Сформулировано пользователю как "чисто std/супервизор.ps доработка, без
VM-изменений" — **оказалось неверно ДВАЖДЫ** в процессе, оба раза
подтверждено эмпирически ДО реализации (не догадками):

1. Обёртка вокруг `супервизор.супервизировать(...)` (нужна была раньше,
   т.к. `запусти` не спавнил module-qualified вызовы напрямую) даёт
   handle с НЕтипизированным `T` — `ctx.process_message_types[sym]`
   (Стадия 24) заполняется ТОЛЬКО для функции, которая САМА вызывает
   `получить()`/`себя()` в СВОЁМ теле, не распространяется через цепочку
   вызовов. `отправить(handle, "любая строка")` на такую обёртку проходил
   тайпчек молча (подтверждено `/tmp`-сценарием ДО правки: второй,
   несовпадающий по типу `отправить` на тот же handle падал с Type Error
   только потому, что ПЕРВЫЙ УЖЕ зацементировал T как `Строка`).
2. Первая версия дизайна (единый library-side relay-процесс ВНУТРИ
   `супервизировать()`, форвардящий `получить_сигнал()` как обычное
   сообщение) — **сломана**: `наблюдать(proc)` регистрирует ТЕКУЩИЙ
   процесс наблюдателем, а вызывает его `ДочерняяЗадача.запустить()`
   (в файле ПОЛЬЗОВАТЕЛЯ), выполняющийся СИНХРОННО внутри процесса
   супервизора — значит наблюдателем оказывается супервизор, а НЕ
   отдельный relay, чьи собственные (вечно пустые) сигналы никогда не
   получат уведомления. Подтверждено минимальным репро (зависание,
   100% CPU, `/tmp/self_msg_test3.ps`) ДО переделки дизайна.

**Языковой фикс — `запусти Модуль.функция(...)`** (qualified-форма).
Раньше `запусти` жёстко требовал bare-`Ident_Expr` callee
(`infer_spawn_expr`). Parser/resolver уже пропускали ЛЮБОЙ `Call_Expr`
(в т.ч. с `Property_Expr`-callee) — блокер целиком был в typechecker'е/
компиляторе. `Module_Graph.process_message_types` (новое поле, тот же
накопительный паттерн, что `symbol_schemes` — заполняется ПОСЛЕ каждого
модуля, копируется в `new_type_ctx` СЛЕДУЮЩЕГО) даёт вызывающему
модулю доступ к T процесса, вычисленному ПРИ ТАЙПЧЕКЕ модуля-владельца
(без повторного `ensure_body_checked` — экспортирующий модуль уже
ПОЛНОСТЬЮ протипизирован раньше по топологическому порядку графа).
Компилятор резолвит `fn_ptr` для `Property_Expr`-callee тем же паттерном,
что уже применяет для обычного `Property_Expr`-как-значения.

**Рабочий дизайн — per-child relay, а НЕ единый library-side relay.**
`ДочерняяЗадача.запустить()` теперь принимает
`супервизор: Процесс(СобытиеСупервизора)` — реализация обязана (после
спавна ребёнка) запустить СВОЙ монотипный (не generic — cross-module
generic-инстанциация `Процесс(T)`-параметра эмпирически ПОДТВЕРЖДЕНА
сломанной: `collect_free_infer_vars`/`generalize` не обходят
`.Process`-тип вообще, T "цементируется" на первый call site — обойдено
тем, что relay пишет ПОЛЬЗОВАТЕЛЬ, монотипно, а не библиотека generic-ом)
relay-процесс, который САМ зовёт `наблюдать(proc)` (значит наблюдателем
корректно становится ОН) и форвардит РОВНО один сигнал супервизору
обычным сообщением. `супервизировать()` благодаря этому УПРОСТИЛАСЬ
обратно до простого цикла на `получить()` — библиотека больше не спавнит
НИКАКИХ вспомогательных процессов сама. `себя()`, вызванный ОДИН раз в
начале `супервизировать()`, пиннит T на `СобытиеСупервизора` (делает
qualified-`запусти` типобезопасным для внешнего вызывающего) и даёт
хэндл, который нужно прокинуть в каждый `.запустить(это_супервизор)`.

**`СобытиеСупервизора`** — единый канал: `Сигнал(id, причина)` (от
relay'ев), `ДобавитьЗадачу(ДочерняяЗадача)`, `УбратьЗадачу(индекс)`.
Удаление — tombstone (`id_ов[i] = -1`; у `Массив` нет метода удаления по
индексу; реальные id процессов всегда ≥0, монотонно растут от 0 — `-1`
никогда не совпадёт с настоящим упавшим id), не физическое удаление —
индексы остальных детей не сдвигаются. Group-restart цикл
(`ВсеЗаОдного`/`ОстальныеЗаОдним`) пропускает tombstone-записи явным
guard'ом.

**Побочно найдено и исправлено — НЕ являющееся частью этой задачи, но
блокировавшее её**: `resolve_variant_ctor` (конструктор варианта
перечисления, напр. `СобытиеСупервизора.ДобавитьЗадачу(значение)`)
использовал `infer_expr`+`unify_types` напрямую для аргументов, В ОБХОД
`check_expr` — значит НИКОГДА не записывал coercion в `ctx.
interface_casts`, которую компилятор читает, чтобы обернуть struct/enum-
значение в `Interface_Value` при передаче в интерфейсное поле. Ни один
вариант перечисления в кодовой базе ДО `ДобавитьЗадачу(ДочерняяЗадача)`
не имел интерфейсного поля — не всплывало. Рантайм падал с "Runtime
Error: попытка вызвать интерфейсный метод у не-интерфейса" на первом же
вызове метода через "сырое" (необёрнутое) значение. Исправлено —
`resolve_variant_ctor` теперь делает ТУ ЖЕ coercion-проверку, что
`check_expr`.

**Побочно найдено, НЕ баг, задокументировано в коде/докстрингах**:
детерминированный (не флакующий — воспроизводится 100% для конкретного
источника) артефакт порядка планировщика — если единственный ребёнок
падает НЕМЕДЛЕННО на сообщение, отправленное СИНХРОННО в том же вызове
`запустить()`, worker (спавнится первым, ниже по индексу
`vm.processes`) успевает выполниться и упасть РАНЬШЕ, чем planировщик
впервые дойдёт до его relay (спавнится вторым). `наблюдать()` на уже
мёртвую цель — штатный (Стадия 38) синтетический сигнал "процесс уже не
существует" вместо реального текста паники worker'а — restart/лимит-
логика на это НЕ влияет (`Есть(причина)` обрабатывается одинаково
независимо от текста), меняется только диагностический текст. Не
исправлено (потребовало бы либо relay, спавнящего worker'а САМ — теряет
синхронный возврат id, либо доп. ack-раунд через вложенный `получить()`
— риск misrouting сообщений при гонке с внешними dynamic-add/remove
командами) — обновлён текст ОДНОГО существующего теста
(`test_qualified_impl_supervisor_restarts_then_gives_up`) под новое,
тоже полностью корректное поведение. Фикстуры с НЕСКОЛЬКИМИ детьми
(`supervisor_one_for_all`/`rest_for_one`) НЕ подвержены — к моменту
паники это уже рестарт ПОСЛЕ `swap-remove`-перетасовки `vm.processes`,
relay гарантированно успевает первым.

**Файлы**: `core/resolver.odin` (`Module_Graph.process_message_types`),
`core/type_cheker.odin` (`new_type_ctx` копирование поля,
`infer_spawn_expr` → `infer_spawn_local_call`/`infer_spawn_qualified_
call`, `resolve_variant_ctor` interface-coercion фикс),
`core/module_loader.odin` (`resolve_and_typecheck_all` накопление поля),
`core/compiler.odin` (`case ^Spawn_Expr:` Property_Expr-ветка),
`std/супервизор.ps` (`СобытиеСупервизора`, `ДочерняяЗадача.запустить()`
новая сигнатура, упрощённый `супервизировать()`, `обработать_добавление`/
`обработать_удаление`), `fixtures/supervisor_*.ps` (обновлены под новую
сигнатуру + 2 новых фикстуры на dynamic add/remove), `fixtures/spawn_
qualified_*.ps` (4 новых, языковой фикс отдельно от supervisor'а),
`test_supervisor.ps` (демонстрирует qualified-запусти + dynamic add),
`docs/src/language/processes.md` (расширен раздел "Супервизия" +
подраздел "Динамические дети", дополнена "Порождение процесса").

**Верификация**: 6 новых e2e-тестов (2 позитивных на qualified-запусти,
3 негативных — плохой модуль/не-функция/неверный тип сообщения, 2 на
dynamic add/remove супервизора) + обновлён текст 1 существующего.
`odin test ./core -define:ODIN_TEST_THREADS=1` — 188/188 (0 регрессий).
Ручные smoke-тесты всех сценариев (позитив/негатив qualified-запусти,
все 5 supervisor-фикстур, dynamic add/remove) через `/tmp/panos_bin` —
подтверждено. native/lsp/wasm сборки чисты.

### Стадия 46 ✅ — Временное окно лимита рестартов (модуль `время` + sliding window)

**Статус**: по запросу ("продолжай супервизию" → "временное окно лимита
рестартов") — закрывает ПОСЛЕДНИЙ пункт `language-fails`/ROADMAP
§Стадия 45: лимит рестартов супервизора был простым счётчиком за ВСЮ
жизнь супервизора, не "N рестартов за M секунд" (OTP-семантика). Grilled
и подтверждено: модуль `время` шире минимума (и `монотонно_мс()`, и
`сейчас_мс()`, не только то, что нужно для окна), алгоритм окна —
sliding window (не fixed window, точнее на границе).

**Расследовано заранее (Explore-агент, файл:строка) до реализации**:
новый builtin-модуль = 4 точки правки, все в `core/stdlib.odin`/
`core/vm.odin` (`is_builtin_module_name`, `builtin_export_type`,
`ensure_builtin_module`, `call_builtin`) — LSP переиспользует тот же
`core`-пакет, третьей точки регистрации нет. `core:time` компилируется
и работает под `-target:js_wasm32` БЕЗ доп. JS-обвязки (`time_js.odin`,
`#+build js`, уже реализует `_now`/`_tick_now` через штатный
`odin_env` — та же обвязка, что уже даёт вендоренный `demo/odin.js`) —
в отличие от `core:os`/`core:net`, `время` НЕ потребовал native/wasm
split (`core/vm_io_native.odin`/`core/vm_io_wasm.odin`, нужный тем
двум). Оба факта подтверждены native+lsp+wasm сборками ДО начала работы
над supervisor-частью — investigation оказался полностью точен.

**`время.монотонно_мс()`/`время.сейчас_мс()`** — `VM.monotonic_epoch`
(`time.Tick`, выставляется один раз в `new_vm`) даёт первую, `time.now()`
+ `time.to_unix_nanoseconds` — вторую. Обе возвращают `Число` (не
`Целое`) — непрерывная величина/измерение, не дискретный счётчик.
Никаких изменений планировщика не потребовалось — `run_scheduler`
чисто message-driven, окно проверяется СИНХРОННО в момент обработки
краша, не нужен таймер/sleep-примитив (которого в panos и не было).

**`std/супервизор.ps` — sliding window**. Сигнатура `супервизировать`
получила четвёртый аргумент `окно_мс: Число` (breaking, как и в каждой
предыдущей стадии супервизора — 2→3→4 аргумента). Целочисленные
счётчики (`Целое`) заменены списками таймстампов (`Массив(Число)`,
`время.монотонно_мс()` — монотонные, иммунны к переводу системных
часов): per-child `счётчики: Массив(Массив(Число))` для `ОдинЗаОдного`,
единый `групповые_рестарты: Массив(Число)` для `ВсеЗаОдного`/
`ОстальныеЗаОдним`. Новый helper `отфильтровать_недавние` — та же
техника "пересобрать массив" (у `Массив` нет удаления по индексу), что
tombstone-фильтрация в Стадии 45.

**Побочно найдено и отброшено — первая версия теста на sliding window
была методологически неверна.** Изначальный дизайн (супервизор без
бюджета крахов, окно=0.1мс, макс_рестартов=5, ожидание что "окно
никогда не накопит лимит → бесконечный рестарт → дедлок, раз ничего
больше не происходит") ПРОШЁЛ 8/8 прогонов, но при инструментальной
проверке (временный `ввод_вывод.печать` внутри `обработать_краш`)
оказалось, что дедлок наступал ПОСЛЕ ~2660 циклов рестарта — тест
проверял исчерпание системных ресурсов после тысяч спавнов процессов,
а не логику окна как таковую (могло вести себя совершенно иначе на
другой машине/аллокаторе — не детерминированное поведение по замыслу,
а везение). Переделано на БЮДЖЕТНУЮ версию: ребёнок падает РОВНО 3
раза (мутируемое поле `осталось` в задаче-структуре, декрементируется
в `запустить()`, после исчерпания триггер-сообщение больше не
отправляется), `макс_рестартов=2` (старая, Стадия 45, "за всю жизнь"
семантика запаниковала бы на 3-м крахе — подтверждено вручную на том
же сценарии с широким окном), окно=0.001мс (на 2+ порядка уже реального
времени цикла краш→relay→сигнал→рестарт). Ровно 3 цикла, ~25мс,
15/15 прогонов чисто.

**Файлы**: `core/vm.odin` (`import "core:time"`, `VM.monotonic_epoch`,
`case "время::монотонно_мс"`/`"время::сейчас_мс"` в `call_builtin`),
`core/stdlib.odin` (модуль `время`), `std/супервизор.ps` (новая
сигнатура, `Массив(Массив(Число))`/`Массив(Число)` вместо счётчиков-
`Целое`, `отфильтровать_недавние`), `fixtures/supervisor_*.ps` (все 5,
под новую 4-арг сигнатуру — широкое окно 60000мс, не меняет поведение
Стадии 45) + `fixtures/supervisor_narrow_window_fixture_main.ps` (новая,
бюджетный sliding-window тест), `test_supervisor.ps` (та же 4-арг
сигнатура), `docs/src/language/basic-types.md` (новый раздел "Время" —
`время` builtin-модуль, не `std/*.ps`-библиотека, не подходит под
`standard-library.md`), `docs/src/language/processes.md` (раздел
"Супервизия" — `окно_мс`/sliding window).

**Верификация**: 3 новых e2e-теста (`время.монотонно_мс()` растёт,
`время.сейчас_мс()` правдоподобен, sliding window реально
консультируется — не молча игнорируется) + обновлён текст 5
существующих (новый "N за Mмс" суффикс в panic-сообщениях). `odin test
./core -define:ODIN_TEST_THREADS=1` — 191/191 (0 регрессий), narrow-
window тест дополнительно прогнан изолированно 5/5 (~25мс каждый) и
основная фикстура вручную 15/15 — детерминированно, без флаков. native/
lsp/wasm сборки чисты (включая `core:time` под `-target:js_wasm32` —
подтверждено, не только по investigation).

---

### Стадия 48 ✅ — Замыкания (value-capture)

**Статус**: по запросу ("реализовать closures"). Грилинг (AskUserQuestion):
захват по значению (snapshot на момент создания лямбды), не по ссылке
(боксед upvalue-ячейки) — совместимо с copy-on-send actor-model
философией без лишних решений про "что происходит при отправке
замыкания другому процессу". До этой стадии лямбда, ссылающаяся на
локал внешней функции, резолвилась (resolver уже разрешал имя через
scope-chain), но давала `Compiler Error` — разрыв был чисто в
компиляторе/VM.

**Механизм захвата**: `core/resolver.odin` — новый upvalue-resolution
walk (`lookup_symbol_tracking_captures`, тот же алгоритм, что в Lua),
отслеживающий пересечение границ активных лямбд (`Resolver_Ctx.
lambda_stack`) во время резолва `Ident_Expr`; результат —
`lambda_captures: map[Expr][dynamic]Symbol_Id` на Lambda_Expr. Работает
корректно для произвольной глубины вложенности (лямбда-в-лямбде —
символ транзитивно распространяется через все промежуточные уровни).

**Рантайм**: новый Value-вариант `^Closure_Value` (`fn: ^Compiled_
Function`, `captured: [dynamic]Value`, GC-managed — в отличие от голой
`^Compiled_Function`, которая живёт в глобальном реестре весь процесс).
Два новых опкода — `.Build_Closure` (снимает N значений со стека,
строит Closure_Value) и `.Get_Captured` (читает `frame.closure.
captured[idx]`). Некапчурящие лямбды (подавляющее большинство
существующего кода — компараторы в `std/коллекции.ps` и т.п.)
компилируются РОВНО как раньше (голая `^Compiled_Function`-константа,
обычный `.Call`) — новый путь активируется только при непустом
capture-списке, минимизирует blast radius изменения.

**GC**: `^Closure_Value` — 5 стандартных точек (`get_header`/`mark_
value`/`value_size`/`pool_release`/`gc_new`-диспетчер), тот же метод,
которым добавлялись `Process_Value`/`Foreign_Function` — Odin's
exhaustive switch сам указал все места. `message_deep_copy` (copy-on-
send) получил case для `^Closure_Value` — captured копируется глубоко
при отправке замыкания другому процессу, тем же механизмом, что
Array_Value/Map_Value — никакого общего мутируемого состояния между
процессами.

**Присваивание захваченной переменной внутри лямбды** — `Type Error`
("захваченная переменная неизменяема внутри лямбды"), не тихий no-op —
проверка в `type_cheker.odin` рядом с существующей Стадия 27
immutable-параметр диагностикой (новый `Type_Ctx.current_lambda_stack`).

**`std/супервизор.ps`**: докstring обновлён (снят устаревший "Compiler
Error" на захват) — архитектура (конфигурация в полях структуры,
реализующей `ДочерняяЗадача`) НЕ изменена: гетерогенные типы сообщений
всё равно требуют номинальной диспетчеризации через интерфейс, замыкание
само по себе vtable не даёт, а struct-поля уже полностью решают задачу
"передать параметры конкретному запустить()".

**Верификация**: 6 новых e2e-тестов (базовый захват, snapshot-семантика
против мутации, множественный захват, Type Error на присваивании,
транзитивный захват через вложенные лямбды, отправка замыкания другому
процессу с deep-copy) + ручной прогон (создать_счётчик(10)/создать_
счётчик(20) — независимые замыкания дают 10+20=30). `odin test ./core
-define:ODIN_TEST_THREADS=1` — 201/201 (0 регрессий). native/lsp/wasm
сборки чисты.

**Явно не сделано**: типизированные замыкания как значения структурных
полей (можно хранить `функ() -> T` в переменной/аргументе — это уже
работало до этой стадии — но не в поле `структура`, отдельно не
проверялось); замыкания как generic type-параметр.

---

### Стадия 49 ✅ — FFI: `КСтрока` + `Указатель(T)`

**Статус**: по запросу ("завершить FFI"), грилинг сузил объём до
"строки + указатели" (не весь оставшийся объём ROADMAP §Стадия 8 —
`ff_структура`/callback'и/SIGSEGV recovery/другие платформы по-прежнему
открыты). Архитектура для `Указатель(T)`/`Pointer_Value`/владения была
продумана заранее (см. §Стадия 8, "Конкретные точки в текущем коде") —
прецеденты: `Процесс(T)` для типа, `File_Value`/`Socket_Value` для
Value+finalizer.

**Грамматика**: `parse_foreign_int_width` обобщена в `parse_foreign_
marshal_type` — три формы внутри `внешний`-сигнатуры: `Целое(32|64)`
(как в Стадии 47), `КСтрока` (голый идентификатор), `Указатель(T)` (T —
обычный `parse_type`). Возвращаемый тип принимает опциональный постфикс
владения — `-> Указатель(Целое) свой`/`чужой` (default —
`чужой`, безопасный дефолт: никогда не освобождать чужую память) —
ни один из идентификаторов (`Целое`/`КСтрока`/`Указатель`/`свой`/
`чужой`) не стал новым keyword'ом лексера, распознаются строковым
сравнением только внутри `внешний`-сигнатуры, тот же приём, что уже был
у `Целое`.

**`Указатель(T)` — настоящий panos-тип**, не только FFI-локальный
синтаксис: новый `Type_Kind.Pointer` + `new_pointer_type`, ветка
`Type_Generic`-цепочки в `resolve_type_node` (рядом с `Процесс(T)`) —
`T` чисто фантомный (не разыменовывается в этом срезе), даёт
типобезопасность (`Указатель(Файл)` ≠ `Указатель(Сокет)`). `Pointer_
Value` — новый Value-вариант (`ptr: rawptr, owned: bool`), те же 5
GC-точек, что у Closure_Value (Стадия 48). `pool_release` освобождает
через libc `free()` ТОЛЬКО если `owned == true` (`vm_ffi_native.odin`'s
`pointer_free`, `foreign import "system:c"` — libc, не третьесторонняя
библиотека, не нарушает политику "вендорить всё"). `message_deep_copy`
НЕ дублирует `Pointer_Value` при отправке процессу (та же категория,
что File/Socket — дублирование wrapper'а вокруг ОДНОГО адреса с
owned=true дало бы double-free) — Стадия 50 подтвердила это безопасным
by-construction (не открытым риском): у panos ОДИН общий `vm.gc` на
весь VM, расшаренный wrapper освобождается ровно один раз, когда
недостижим ИЗ ВСЕХ процессов сразу (см. `test_ffi_pointer_owned_sent_
to_process_no_double_free`, e2e_test.odin).

**Маршаллинг**: `Foreign_Function`'s `param_widths: []int`/`return_
width: int` обобщены в `Foreign_Marshal_Kind`-based поля (`Int32,
Int64, CString, Pointer`). `КСтрока` — panos `Строка` копируется в
null-terminated C-буфер на время вызова (borrowed convention — если
C-функция сохраняет указатель дольше своего вызова, это вне гарантий
этого среза); возврат `char*` копируется в НОВУЮ `Panos_String` (panos
никогда не заимствует чужую C-память для строк). `Указатель(T)` —
прямой passthrough `rawptr`, без копирования. `ffi_type_pointer`
(libffi-дескриптор) общий для `КСтрока`/`Указатель(T)` — C ABI обоих
одинаков (адрес-слово), различие только в том, что panos кладёт по
этому адресу.

**Верификация**: 4 новых e2e-теста (`strlen` — КСтрока-параметр,
`strdup` round-trip через кириллицу — КСтрока-возврат, `malloc` с
`свой` — не падает, `getenv` без владения — не пытается
освободить чужую память) + ручной прогон (`strlen("hello world")` = 11,
`strdup("привет мир")` возвращает точную копию). `odin test ./core
-define:ODIN_TEST_THREADS=1` — 205/205 (0 регрессий). native/lsp/wasm
сборки чисты.

**Явно не сделано** (см. §Стадия 8 выше на полный список): `ff_
структура` (C ABI layout), callback'и (`ffi_closure`/trampolines),
SIGSEGV recovery, `Число(32)`/`Число(64)` (float-ширины), платформы
кроме `darwin-arm64`.
...
**Статус**: Стадия 49 задокументировала как ОТКРЫТЫЙ риск — отправка
владеющего (`свой`) `Указатель(T)` другому panos-процессу через
`отправить()` НЕ дублирует сам C-указатель (`message_deep_copy`
расшаривает `Pointer_Value`-wrapper, как File/Socket), и первоначальное
опасение было: если оба процесса переживут получателя своими путями,
`free()` мог бы вызваться дважды на одном адресе.

**Расследование показало: это ложная тревога**, не настоящий риск —
panos'овские lightweight-процессы (Стадия 24) НЕ независимые OS-процессы
с раздельной памятью, это кооперативные green-thread'ы внутри ОДНОГО
`VM`-struct'а с ОДНИМ общим `vm.gc: GC_State`. Расшаривание ссылки на
`Pointer_Value`-wrapper (вместо глубокого копирования) безопасно by
construction: GC освобождает wrapper РОВНО ОДИН РАЗ, когда он становится
недостижим из корней ВСЕХ процессов одновременно — двух `free()`-вызовов
физически быть не может, потому что нет двух независимых куч.

**Верификация**: новый e2e-тест `test_ffi_pointer_owned_sent_to_process_
no_double_free` (`core/e2e_test.odin`) — `malloc(64)` с `свой`,
указатель отправляется спавненному процессу, `force_gc`/`gc_stats`
подтверждают `freed_last_run > 0` без паники/double-free. Комментарий в
`core/vm.odin` (`message_deep_copy`) и формулировка в этом файле
скорректированы — риск закрыт документацией, не кодом (код уже был
корректен).

---

### Стадия 51 ✅ — FFI-примитивы для raylib + удаление `графика` из core

**Статус**: по запросу ("довести FFI до состояния, когда можно вынести
raylib в отдельную библиотеку") — грилинг (`AskUserQuestion`) выбрал
максимальный объём: примитивы ДОБАВИТЬ И убрать графику из core
ПОЛНОСТЬЮ (не просто "добавить примитивы, оставить графика как есть").
Единственная оставшаяся точка, нарушавшая политику "вендорить всё/panos
самодостаточен" — core-builtin модуль `графика` (Стадия 4, статические
Odin-биндинги поверх `vendor:raylib`, требующие системного `brew install
raylib` на этапе СБОРКИ самого `panos`) — устранена целиком.

**`Foreign_Marshal_Kind` расширен** (`Int32, Int64, CString, Pointer` →
`+ Void, Int8, Float32, Float64, Struct`, 9 вариантов) — тот же приём,
что все предыдущие FFI-стадии: Odin's exhaustive switch сам указывает
каждую точку, которую нужно обновить (parser/type_cheker/ffi_bindings/
vm_ffi_native). `Целое(8)` маршаллится КАК БЕЗЗНАКОВЫЙ `u8` (не `i8`) —
поймано живым тестом против настоящего raylib (`Fade` дал 126 вместо
382 с сигнатурным `i8`), не автоматической проверкой компилятора.
`Пусто` как тип возврата `внешний`-функции — не было в исходном плане,
добавлено по ходу как естественное механическое расширение (8 из 12
нужных raylib-функций возвращают `void`).

**`ff_структура`** — четвёртая форма `тип X = ...` рядом со `структура`/
`перечисление`/`интерфейс` (новый `TokenKind.FFStruct`, лексер-keyword
`"ff_структура"`, `Struct_Decl.is_ffi`). Поля — только `Целое(8|32|64)`/
`Число(32|64)` (плоский layout, без вложенности/строк/указателей в этом
срезе). На panos-стороне — ОБЫЧНЫЙ именованный конструктор (`Vector2(1.0,
2.0)`, Стадия 36) и `.`-доступ, переиспользует `Aggregate_Value` —
НОЛЬ новых GC-точек. `Type` получил `ffi_field_kinds`/`ffi_composite`/
`ffi_offsets` — составной `^Ffi_Type` и byte-offset'ы полей строятся
лениво при первом реальном FFI-вызове через `ffi_prep_cif`/
`ffi_get_struct_offsets` (offset'ы можно спросить у libffi ТОЛЬКО ПОСЛЕ
того, как структура уже поучаствовала в успешном `ffi_prep_cif` —
порядок операций важен).

**Удаление `графика`**: `core/vm_graphics_native.odin`/`core/vm_graphics_
wasm.odin` удалены целиком, регистрация модуля в `core/stdlib.odin` и
диспетчинг `call_builtin_graphics` в `core/vm.odin` убраны. Подтверждено
`otool -L` — raylib больше не слинкована в бинарь `panos` вообще.

**`demos/raylib.ps`** — новая ОБЫЧНАЯ panos-библиотека поверх `внешний`/
`ff_структура`, точная замена прежнего API `графика` (12 обёрточных
функций + геттеры кодов клавиш). Путь к библиотеке захардкожен на macOS/
Apple Silicon + `brew install raylib`
(`/opt/homebrew/opt/raylib/lib/libraylib.dylib`) — честно
задокументированный пробел, тот же принцип, что `external/libffi/
README.md`. Полный путь потребовал фикса `core/resolver.odin`'s
`foreign_library_filename` — раньше ВСЕГДА дописывал платформенное
расширение, теперь проверяет `strings.has_suffix`, чтобы не задваивать
его для уже-полных путей (raylib не лежит на стандартных путях поиска
dyld, в отличие от libc). `demos/pong.ps` портирован на `raylib.ps`
(`импорт графика` → `импорт raylib`, bare-туплы `(x, y)`/`(r,g,b,a)` →
именованные конструкторы `raylib.Vector2(...)`/`raylib.Color(...)`) —
визуально проверен (`./panos demos/pong.ps`, окно открывается, рендер
идёт без паники).

**Верификация**: 3 новых e2e-теста против НАСТОЯЩЕГО raylib
(`Vector2Add` — float-структура round-trip, `Fade` — Целое(8)-структура,
`SetTargetFPS` — void-возврат) — 209/209 всего, 0 регрессий.
native/lsp/wasm сборки чисты.

**Явно не сделано** (см. §Стадия 8 выше): callback'и (`ffi_closure`),
SIGSEGV recovery, вложенные `ff_структура`/строки/указатели как поля,
платформы кроме `darwin-arm64`.

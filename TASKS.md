...
возможны). 23 (Печатаемое ✅/Арифметика ✅/Копируемое ✅/Хешируемое/
Итерируемое; По-умолчанию выброшена — см. ниже) — Хешируемое/
Итерируемое остаются кандидатами, требуют investigation до плана. 24 (lightweight
...
- [x] **Итерируемое (Iterable)** — investigation завершён, реализация
      ЗАБЛОКИРОВАНА Стадией 28 (см. ниже). Grilled: iterator protocol
      (`итератор()`/`следующий() -> Опция(T)`), не indexable. Explore-
      агент нашёл: for-in десахаривается ЦЕЛИКОМ на parse-time
      (parser.odin:1176-1295), под `.длина()`+`[idx]`. Моя правка
      поверх находки агента: iterator-protocol форма несовместима с
      УЖЕ десахаренными узлами (Call_Info-sugar не спасает — другой
      control flow, не другой опкод на тех же узлах). Решение:
      необесахаренный `For_In_Stmt`, десахаривание переносится в
      compiler.odin (где уже есть типы). Заблокировано генерик-
      интерфейсами (T как независимый параметр метода — Self-фикс не
      покрывает)
- [ ] **Хешируемое (Hash)** — тяжёлый тир, отдельная подсистема
      (рантайм-хеш `Соответствие` в vm.odin, не typecheck/compile слой
      Стадии 22). Своя investigation, свой Explore-проход
...
Refs: [ROADMAP §Стадия 28](ROADMAP.md#стадия-28-найдена-при-investigation-итерируемого-не-исследована--generic-интерфейсы).

Жёсткий prerequisite для Итерируемого (Стадия 23), не самостоятельная
инициатива — недостающая под-фаза уже в основном готовой Стадии 7
(Generics). Беглый просмотр (НЕ formal Explore) нашёл симметричные
touch-точки:

- [ ] `core/parser.odin:78-83` (`Interface_Decl`) — нет `type_params`,
      добавить + вызов `parse_type_params(p)` в `parse_interface_decl`
      (симметрично `parse_struct_decl`, parser.odin:1011-1013)
- [ ] `core/type_cheker.odin:1351-1356` (ПРОХОД 1) — `Type{kind =
      .Interface}` без `generic_origin` (в отличие от Struct/Enum
      рядом) — выставить симметрично
- [ ] `core/type_cheker.odin:1446-1455` (ПРОХОД 2) — резолв
      `interface_methods` без `make_decl_type_params`/`try_generalize`
      обёртки, которую уже используют Struct_Decl (1396-1420)/Enum_Decl
      (1468+) — симметрично применимо в принципе
- [ ] **Не исследовано**: семантика `try_generalize`/инстанциации для
      интерфейса — у структур/enum'ов схема нужна для call-site
      inference конструктора, интерфейс не "вызывается". Использование
      иное: `реализация X(Тип) для Y` и аннотация типа параметра
      `f(x: X(Тип))` — не проверено, поддерживает ли `parse_impl_decl`
      явный типовой аргумент у имени интерфейса вообще, и как резолвится
      инстанциация в обоих контекстах (вероятно НЕ путь call-site
      inference конструкторов)

**Prerequisite**: Стадия 7 (Generics) — фактически готова, это —
недостающий кусок её охвата.

**Effort**: не оценено — instantiation-семантика интерфейсов не
исследована вообще, реальный объём может быть больше 3 найденных точек.

...
— кандидаты, требуют investigation до плана. 24 (lightweight processes,
Elixir/Akka-style actor model — дважды пересмотрена, CSP-каналы
...
26 (`panos mod` — встроенный пакетный менеджер, go mod style) — заведена,
открытые design-вопросы не решены. **Приоритет (grilled)**: 22
следующая, 23/24/25/26/4/8 — после неё. Type_Id/SoA (остаток Стадии 5)
— отдельная перформансная задача без
явного триггера.
...
Refs: [ROADMAP §Стадия 26](ROADMAP.md#стадия-26-заведено-не-исследовано--panos-mod-встроенный-пакетный-менеджер).

Подкоманды у `panos` (go mod style, один бинарник) — **решено явно**:
НЕ отдельный бинарник `pan` (существующая заготовка
`/Users/gaidar/dev/panosiki/pan/`, 0 коммитов — становится неактуальной,
план живёт здесь). Переиспользует уже готовое: `модули/`-fallback в
резолвере (resolver_import_native.odin:12) — vendoring зависимостей
туда работает без правок в резолвере; `std/кодирование/toml.ps` —
готовый TOML-парсер для манифеста.

- [ ] Decide: Minimal Version Selection (Go-style, полноценный) vs
      плоский список прямых зависимостей без транзитивного резолва (v1)
- [ ] Decide: только git clone/checkout по тегу, без registry-сервера —
      имя пакета = git URL?
- [ ] Decide: semver git-теги, с Go-style major-в-пути-импорта
      конвенцией или без
- [ ] Decide: `панос.toml`/`панос.lock` (кириллица) vs `panos.toml`/
      `panos.lock` (латиница, внешняя конвенция)
- [ ] Decide: манифест парсит нативный Odin-хост (до старта VM) или
      panos-код (`std/кодирование/toml.ps`) вызванный из хоста
- [ ] Investigate (после решений выше): конкретные touch-точки —
      main.odin (новые subcommands), формат `модули/`-vendoring layout
      для многофайловых пакетов

...
решения grilled (три раунда), touch-точки требуют Explore. 29-46
...
групповым рестартам супервизора + link-примитив `связать` + qualified
`запусти Модуль.функция(...)` + динамические дети супервизора
(`ДобавитьЗадачу`/`УбратьЗадачу` через per-child relay-процессы) +
модуль `время` + временное окно лимита рестартов, sliding window —
актёрная модель/супервизия ЗАКРЫТА ПОЛНОСТЬЮ). **Приоритет (grilled)**:
24/26/4/8 — следующие кандидаты. Type_Id/SoA
(остаток Стадии 5) — отдельная перформансная задача без явного
триггера.
...
По запросу ("продолжай супервизию" → "временное окно лимита
рестартов") — закрывает ПОСЛЕДНИЙ пункт `language-fails`/ROADMAP
§Стадия 45. Актёрная модель/супервизия теперь закрыты полностью.

- [x] Новый builtin-модуль `время` (`монотонно_мс()`/`сейчас_мс()`) —
      `is_builtin_module_name`/`builtin_export_type`/`ensure_builtin_
      module` (stdlib.odin), `call_builtin` (vm.odin), `VM.monotonic_
      epoch` выставляется один раз в `new_vm`
- [x] Investigation до реализации подтвердил: `core:time` работает под
      `-target:js_wasm32` БЕЗ native/wasm split (в отличие от
      `core:os`/`core:net`), LSP не нуждается в отдельной регистрации
      (переиспользует `core`-пакет), планировщик не нуждается в
      изменениях (окно проверяется синхронно, не таймером)
- [x] `std/супервизор.ps`: 4-й аргумент `окно_мс: Число` (breaking, как
      и в каждой предыдущей стадии супервизора), счётчики (`Целое`) →
      списки таймстампов (`Массив(Число)`/`Массив(Массив(Число))`),
      новый helper `отфильтровать_недавние` (та же техника, что
      tombstone-фильтрация в Стадии 45 — у Массива нет remove-at)
- [x] Первая версия теста на sliding window (без бюджета крахов, окно
      0.1мс) отброшена — эмпирически проверяла исчерпание системных
      ресурсов после ~2660 циклов рестарта, не логику окна.
      Переделана на бюджетную версию (ребёнок падает ровно 3 раза,
      окно 0.001мс) — быстро (~25мс), детерминированно, 15/15 без
      флаков

3 новых e2e-теста (монотонность/правдоподобность времени, sliding
window реально фильтрует) + обновлён текст 5 существующих
(panic-сообщения получили суффикс "N за Mмс"). `docs/src/language/
basic-types.md` — новый раздел "Время". `docs/src/language/
processes.md` — раздел "Супервизия" дополнен про `окно_мс`. `odin test
./core -define:ODIN_TEST_THREADS=1` — 191/191 (0 регрессий). native/
lsp/wasm чисто (включая `core:time` под wasm-целью — подтверждено, не
только по investigation).

---

## Стадия 45 ✅ — Динамические дети супервизора (qualified `запусти` + per-child relay)

Refs: [ROADMAP §Стадия 45](ROADMAP.md#стадия-45--динамические-дети-супервизора-qualified-запусти--per-child-relay).

По запросу ("продолжай супервизию" → "динамические дети") — закрывает
предпоследний пункт `language-fails` (остаётся временное окно лимита
рестартов — требует time-модуля с нуля). Сформулировано как "чисто
библиотечная доработка" — оказалось неверно дважды, оба раза
подтверждено эмпирически ДО реализации, не догадками (см. ROADMAP).

- [x] Языковой фикс: `запусти Модуль.функция(...)` (qualified-форма) —
      `Module_Graph.process_message_types` (новое поле, накопительный
      паттерн `symbol_schemes`), `infer_spawn_expr` разделён на
      `infer_spawn_local_call`/`infer_spawn_qualified_call`, компилятор
      резолвит `fn_ptr` для `Property_Expr`-callee
- [x] Обёртка вокруг module-qualified вызова НЕ даёт типобезопасный
      handle (T пиннится только получить()-паттерном ВНУТРИ самой
      спавненной функции, не распространяется через цепочку вызовов) —
      подтверждено эмпирически до правки, значит qualified-запусти
      обязателен, не опционален
- [x] Единый library-side relay ВНУТРИ `супервизировать()` — сломан:
      `наблюдать()` регистрирует ТЕКУЩИЙ процесс, а `ДочерняяЗадача.
      запустить()` выполняется синхронно внутри супервизора — значит
      наблюдателем становится супервизор, не relay (подтверждено
      зависанием на минимальном репро)
- [x] Рабочий дизайн: per-child relay, спавнится САМИМ `запустить()`
      (теперь принимает `супервизор: Процесс(СобытиеСупервизора)`),
      монотипный (НЕ generic — cross-module generic-инстанциация
      `Процесс(T)`-параметра подтверждена сломанной,
      `collect_free_infer_vars`/`generalize` не обходят `.Process`)
- [x] `супервизировать()` упростилась обратно до простого цикла на
      `получить()` — библиотека больше не спавнит вспомогательных
      процессов сама
- [x] `СобытиеСупервизора` — единый канал (`Сигнал`/`ДобавитьЗадачу`/
      `УбратьЗадачу`), удаление — tombstone (`id_ов[i] = -1`, у Массива
      нет remove-at), group-restart цикл пропускает tombstone'ы
- [x] Побочно найден и исправлен pre-existing баг: `resolve_variant_
      ctor` не делал interface-coercion для аргументов конструктора
      варианта (`ctx.interface_casts` не заполнялся) — ни один вариант в
      кодовой базе раньше не имел интерфейсного поля, не всплывало

Побочно найден (НЕ баг, задокументирован) детерминированный артефакт
порядка планировщика: единственный ребёнок, падающий немедленно на
сообщение, отправленное синхронно в `запустить()`, иногда крашится
раньше, чем scheduler дойдёт до его relay — синтетический "процесс уже
не существует" вместо реального текста паники, restart/лимит-логика не
затронута. Обновлён текст 1 существующего теста.

6 новых e2e-тестов (2 позитив/3 негатив на qualified-запусти, 2 на
dynamic add/remove) + 6 новых fixture-файлов. `docs/src/language/
processes.md` — расширена "Супервизия" + новый подраздел "Динамические
дети". `odin test ./core -define:ODIN_TEST_THREADS=1` — 188/188 (0
регрессий). native/lsp/wasm чисто.

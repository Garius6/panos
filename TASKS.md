...
возможны). 23 (Печатаемое/Арифметика/По-умолчанию/Копируемое/Хешируемое/
Итерируемое) — кандидаты, требуют investigation до плана. 24 (lightweight
...
**Найдено попутно (НЕ баг Стадии 22, предсуществующий)**: `кол.отсортировать`
(std/коллекции.ps) ломает generic-инференс, когда T — структура
(`отсортированные[0].x` → "попытка получить поле у не-структуры").
Воспроизведено И БЕЗ Ord/Eq (`a.x < b.x`, только числа) — не связано со
Стадией 22. Не заведено отдельной стадией — не исследовано глубоко.

### Verification

- [x] `odin test ./core` — 88/88 (было 82, +6 новых для этой стадии),
      без регрессий

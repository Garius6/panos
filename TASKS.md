...
17, 18, 19, 20, 21 (10-21 — вне очереди), 5 частично (Symbol_Id + LSP
completions/references/rename; Type_Id отложен). Следующие
разблокированные: 4 (FFI-A), 8 (FFI-B, требует 1 ✓ — finalizers теперь
возможны), 22 (Сравниваемое/Равнозначное — распланировано, не начато).
23 (Печатаемое/Арифметика/По-умолчанию/Копируемое/Хешируемое/Итерируемое)
— кандидаты, требуют investigation до плана. 24 (stackful coroutines) —
ключевые решения grilled и подтверждены, touch-точки требуют Explore
до плана. 25 (интерфейсы для перечислений) — untracked gap, найден при
grilling 22, независим, не исследован. **Приоритет (grilled)**: 22
следующая, 23/24/25/4/8 — после неё. Type_Id/SoA (остаток Стадии 5) —
...
- [ ] Self-тип в интерфейсах: фикс `interface_method_types_match`/
...
- [ ] Grilled: точное сообщение для несовместимых операндов —
...
- [ ] e2e: структура с `реализация Сравниваемое` — `<`/`>`/`<=`/`>=`,
      сортировка через встроенный `<`
- [ ] e2e: структура БЕЗ `реализация Сравниваемое` — `<` даёт понятную
      diagnostic, не крэш
- [ ] e2e: структура с `реализация Равнозначное` (нестандартное сравнение,
      напр. по одному полю) — `==` зовёт `равно`, не структурный путь
- [ ] e2e: структура БЕЗ `реализация Равнозначное` — `==` работает как
      раньше (регрессия старого структурного пути)
- [ ] e2e: Self-фикс отдельно — метод `сравнить`, реально читающий поля
      `другое.x`/`другое.y` (не только typecheck, а исполнение)
- [ ] e2e: две РАЗНЫЕ Сравниваемые структуры (`Точка < Линия`) и
      Сравниваемое-структура с `Число` (`Точка < 5`) — точное сообщение
      про несовместимость, не старое "ожидает два числа"

### Verification

- [ ] `odin test ./core` — без регрессий (базовая линия перед стадией:
      82/82)
...
- [ ] **Печатаемое (Show)** — лёгкий тир (та же инфра, что Стадия 22, без
      operator-sugar части). Investigate: где в vm.odin форматируются
      значения в строку (`вывод.печать`/интерполяция) — та точка, где
      нужен hook на `.вСтроку()`
- [ ] **Арифметика** (`+`/`-`/`*`/`/` overload) — лёгкий тир, тот же
      механизм, что Стадия 22, под `.Plus`/`.Minus`/`.Star`/`.Slash`.
      Осторожно: `.Plus` уже хардкод-диспатчит Число+Число/Строка+Строка
      (type_cheker.odin ~2555) — sugar-ветка встаёт ПОСЛЕ обеих
      существующих проверок
- [ ] **По-умолчанию (Default)** — лёгкий-средний тир, `тип() -> Тип` без
      получателя-от-другого (проще Ord). Investigate: где `Массив.заполнить(N)`
      и подобные требуют явного значения — точка хука
- [ ] **Копируемое (Clone)** — семантика решена (grilled): структуры
      reference semantics по умолчанию (см. ROADMAP §8, живой тест
      `b=a; b.x=99; a.x` → `99`), Копируемое даёт ГЛУБОКУЮ копию
      (рекурсивно, Python-`deepcopy`-стиль). Investigate: где живёт
      структ-присваивание (compiler.odin/vm.odin — `Set_Local`/
      `Set_Property` копируют `^Aggregate_Value` указатель) — точка
      хука под `.клонировать()`
- [ ] **Хешируемое (Hash)** — тяжёлый тир, отдельная подсистема
      (рантайм-хеш `Соответствие` в vm.odin, не typecheck/compile слой
      Стадии 22). Своя investigation, свой Explore-проход
- [ ] **Итерируемое (Iterable)** — тяжёлый тир, архитектурный. `для x в`
      сейчас чистый parser-sugar (Стадия 17), фиксирован на встроенных
      `Массив`/`Соответствие` — пользовательский тип потребовал бы
      переноса части логики на resolver/typecheck уровень

---

## Стадия 24 (grilled, не Explore-исследовано) — stackful coroutines

Refs: [ROADMAP §Стадия 24](ROADMAP.md#стадия-24-grilled-не-explore-исследовано--stackful-coroutines).

Cooperative stackful-корутины поверх байткод-VM. Дёшево именно для этой
VM: `VM.frames`/`VM.stack` (vm.odin:18) уже в куче — suspend/resume без
asm/setjmp/OS-fiber, только смена `current`-указателя. Архитектура
(`Coroutine{frames,stack,state}` + `Scheduler{current,ready}`,
yield = ранний `return` из `execute()`) прошла grilling-сессию
(`/mattpocock-skills:grilling`) — архитектурные РЕШЕНИЯ ниже
подтверждены явно, touch-точки (компилятор/GC/канал-семантика) — ещё
НЕТ, не Explore-исследованы на file:line уровне.

**Решено (grilled)**:
- [x] Single-OS-thread cooperative, БЕЗ реального параллелизма —
      параллелизм осознанно не нужен пока (M:N потребовал бы concurrent
      GC — отдельный проект без текущего задела)
- [x] Блокирующий I/O НЕ кооперируется в v1 — явный non-goal (мотивация
      Стадии 24 — генераторы/CPU-bound конкурентность, не параллельные
      сетевые запросы)
- [x] Синтаксис запуска — `запусти <выражение-вызов>`, statement-form
      без своего блока (panos закрывает блоки `конец`, не `{}`);
      многострочное тело — через уже существующий anon-function литерал
      (`запусти функ() -> Пусто ... конец`)
- [x] Тип канала — `Канал(T)` (форма как у `Опция(T)`/`Массив(T)`)
- [x] Yield — `дай <выражение>`

**Investigate (не решения, а факты, которые нужно найти в коде перед
Порядком работ)**:
- [ ] Компиляция `дай`/`запусти` в новые Opcode (compiler.odin)
- [ ] Типизация `Канал(T)`-операций (`<-`/`->`), интеграция с
      инфраструктурой дженериков (Стадия 7 ✅)
- [ ] GC root-walking по множеству стеков корутин (gc.odin) — сейчас
      один `vm.stack`/`vm.frames`, тривиальный корень
- [ ] Wait-list канала — структура данных, fairness
- [ ] Буферизация канала по умолчанию (0/N)

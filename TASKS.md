...
до плана. **Приоритет (grilled)**: 22 следующая, 23/24/4/8 — после неё.
Type_Id/SoA (остаток Стадии 5) — отдельная перформансная задача без
явного триггера.
...
- [ ] **Копируемое (Clone)** — сначала ДИЗАЙН-обсуждение семантики
      (что такое "копия" при текущей copy-by-value для примитивов +
      ссылочных Array/Map/String внутри структур), потом уже
      investigation

...
pub fn build(b: *std.Build) void {
    ...
    const aot_closures_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("tests/wasm/aot_closures_test.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{.{ .name = "panos_core", .module = core_module }},
        }),
    });
    const run_aot_closures_tests = b.addRunArtifact(aot_closures_tests);
    ...
    aot_step.dependOn(&run_aot_closures_tests.step);

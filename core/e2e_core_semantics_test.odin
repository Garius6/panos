...
// строки.в_байты/кодовая_точка/в_руны/из_рун (см. call_builtin в vm.odin) —
// Go-style []byte(s)/[]rune(s)/rune(s[i])-эквиваленты поверх panos-строк
// (rune-based по умолчанию, но byte-transparent в хранении).
@(test)
test_string_bytes_and_runes :: proc(t: ^testing.T) {
	// "Aя中😀" — 1+2+3+4 = 10 байт, 4 руны, четыре разных ширины UTF-8 разом.
	result, ok := run_code(`
		импорт строки
		...
		функ старт() -> Булево
			...
			пер байты = строки.в_байты(s)
			пер байты_ок = байты.длина() == строки.длина_байт(s)

			пер руны = строки.в_руны(s)
			пер руны_ок = руны.длина() == 4
				и руны.получить(0, 0) == 65
				и руны.получить(1, 0) == 1103
				и руны.получить(2, 0) == 20013
				и руны.получить(3, 0) == 128512

			пер округл_трип = строки.из_рун(руны) == s

			пер codepoint_ок = строки.кодовая_точка("я") == 1103
				и строки.кодовая_точка(s[3]) == 128512

			байты_ок и руны_ок и округл_трип и codepoint_ок
		конец
	`)
	testing.expectf(t, ok, "[строки: байты/руны] стек пуст")
	if ok {
		b, is_bool := result.(bool)
		testing.expectf(t, is_bool && b, "[строки: байты/руны] ожидалось true, получено %v", result)
	}
}

// Регрессия: голый 'возврат' (без значения) перед НОВЫМ statement'ом на
// следующей строке (не ';'/'конец'/EOF — почти любой код) раньше жадно
// парсил этот statement КАК своё возвращаемое значение (parse_return_stmt
// проверял только tok.kind == .Semicolon/.End/.EOF, не Token.nl_before)
// — молча меняя семантику: тело после 'возврат' не только не считалось
// недостижимым, оно реально ИСПОЛНЯЛОСЬ как часть возврата. Найдено
// эмпирически при проверке unreachable-code warning (check_unreachable_
// code, type_cheker.odin) на реальном проекте (../panosiki/pan) — баг
// предсуществующий, не специфичен для этой фичи. Тот же приём, что
// "Скобка в начале statement'а после перевода строки" тестом выше —
// Token.nl_before, core/token.odin.
@(test)
test_bare_return_before_next_statement_does_not_consume_it :: proc(t: ^testing.T) {
	// вспом() -> Пусто: если бы старый баг вернулся, голый 'возврат' внутри
	// если поглотил бы паника(...) как СВОЁ значение (недопустимо для
	// Пусто — типизация упала бы) ЛИБО (для -> Число ниже) паника реально
	// исполнилась бы, роняя весь тест паникой раньше testing.expect_value.
	// Успешный результат — доказательство, что 'возврат' корректно
	// остановил тело ДО паника(...).
	result, ok := run_code(`
		функ вспом() -> Пусто
			если истина тогда
				возврат
				паника("НЕ должно исполниться — поглощено бы старым багом в возврат <значение>")
			конец
		конец
	...
	testing.expect_value(t, result, Value(f64(1)))
}

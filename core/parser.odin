...
parse_type :: proc(p: ^Parser) -> Type_Node {
	...
	if tok.kind == .Ident {
		...
		// (тело функции начинается с обычного выражения в скобках) на этом
		// месте ошибочно читается как generic-тип `Число(1 + 2)` — токен
		// '(' достижим по peek независимо от перевода строки между ним и
		// именем типа. `Массив(Число)` всегда пишется на одной строке —
		// требование "нет nl_before" ничего легитимного не ломает.
...
parse_expr :: proc(p: ^Parser, min_bp: int) -> Expr {
	...
	for {
		...
		// вызов `7(а <= б)`). Та же ошибка, что была в parse_type для
		// `-> Число\n(...)` (см. Token.nl_before) — здесь тот же приём для
		// call-выражений: peek на '(' срабатывал независимо от перевода
		// строки между предыдущим токеном выражения и '('.

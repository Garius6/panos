...
RESERVED_BUILTIN_NAMES := [?]string {
	...
	"убить",
	"связать",
	// Стадия: строковая интерполяция (`"...\(x)..."`) десахаривается на
	// этапе парсинга в цепочку `+` со вставками `встроку(x)` — тот же
	// Печатаемое-диспатч, что `ввод_вывод.печать`/`.строка` (Стадия 23),

...
Token :: struct {
	...
	nl_before: bool,
	// Текст непосредственно предшествующего блока `///`-комментариев
	// (docstring), пусто если такого блока не было (см. Token.nl_before
	// комментарий выше — тот же принцип: side-инфо на токене, который
	// парсер приписывает декларации, см. skip_whitespace_and_comments в
	// lexer.odin).
	doc:       string,
}

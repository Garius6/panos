...
// unify_types(b, a) для всего в этой функции, включая Никогда (обе
// стороны сливаются с чем угодно) и Interface-ветку ниже (порядок
// left/right в ней решает, какая сторона Struct/Enum, а не actual/
// expected — при перестановке аргументов сработает симметричная ветка
// с других сторон вызова). Это НЕ функция для проверки "можно ли
// использовать значение A там, где ожидается B" — та направленная и
// живёт отдельно, см. is_assignable_to (actual-vs-expected: аргументы
// вызова, return, var-декларации). unify_types вызывается тут в трёх
// разных ролях: (1) связывание inference-переменных (bind_infer_var);
// (2) join-точки — если/иначе, элементы массива/match (см.
// emit_constraint/solve_constraints) — там симметричность Никогда
// корректна: пост-обработка в infer_if_expr/infer_match_expr САМА решает
// финальный тип join'а по тому, какая сторона была Никогда, unify лишь
// подтверждает "эти две ветки совместимы"; (3) чистое структурное
// сравнение (тело generic-типа, сигнатура interface-метода и т.п.), где
// направление вообще не имеет смысла. Если понадобится добавить сюда ещё
// один направленный частный случай (как раньше был Никогда) — он,
// скорее всего, должен идти в is_assignable_to, а не сюда.
//

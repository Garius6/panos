...
when #config(PANOS_WASM_BACKEND_TESTS, false) {
	...
	//
	// Область ЭТОГО среза — методы, чей `выбор это` использует ТОЛЬКО
	// wildcard-паттерны (есть/пусто/успех/ошибка — "Есть(_)"/"Успех(_)",
	// ничего не извлекается из payload) — реально работает end-to-end.
	// Методы, чей `выбор это` ИЗВЛЕКАЕТ payload через реальный биндер
	// (значение/получить/причина/... — "Есть(x) -> x") — НЕ покрыты:
	// "это: Опция" (голая, без явных type-аргументов — стандартная форма
	// receiver-параметра метода) резолвится в ОБЩИЙ, НИКОГДА по-настоящему
	// не инстанцируемый шаблонный Type-объект (bind_function_args/
	// current_type_params корректно подставляют T в ПРЯМЫЕ ссылки на T в
	// аннотациях параметров/возврата, но не пересобирают receiver-тип
	// целиком) — lower_pattern (Фаза 2.1, structural extraction через
	// value_type.variants[tag].fields[idx]) поэтому по-прежнему видит T/E
	// абстрактными для ЛЮБОГО РЕАЛЬНОГО биндера внутри `выбор это`,
	// независимо от того, участвует ли T в параметрах или только в
	// возврате (значение$Число — БЕЗ T-параметра вообще — падает ТАК ЖЕ,
	// как получить$Число,Строка — С T-параметром: реальный триггер —
	// биндер в паттерне, не сигнатура). is_wasm_phase1_function отбрасывает
	// такой клон (несколько InferVar-локалей от match-биндеров) — САМ MIR
	// корректен (bytecode-бэкенд работает, символ резолвится верно),
	// просто эта КОНКРЕТНАЯ клонированная функция не входит в WASM-модуль
	// — то же самое "тихо не эмитим, паникуем только если реально
	// вызвана" поведение, что Фаза 1 всегда имела для внепланового кода
	// (см. is_wasm_phase1_function's докстринг) — НЕ регрессия, известная,
	// более глубокая (инстанциация receiver-типа под substitution) задача
	// вне области этой сессии, задокументирована в project-памяти. Опция
	// (Процесс(T)) как T-аргумент — отдельный, тоже известный узкий гэп
	// (record_generic_method_instantiation явно исключает .Process).

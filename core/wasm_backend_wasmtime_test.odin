...
when #config(PANOS_WASM_BACKEND_TESTS, false) {
	...
	// области). Опция(Процесс(T)) как T-аргумент — ВСЁ ЕЩЁ известный,
	// более узкий (чем раньше) гэп: исходный симптом ("это использовался
	// до инициализации") исправлен, но supervisor-фикстуры (актёры не
	// реализованы в этом бэкенде вообще, см. план — Spawn_Instr в
	// панике-листе) блокируют end-to-end проверку через wasm конкретно;
	// байткод-путь (just test) показывает ОТДЕЛЬНУЮ, ещё не найденную
	// downstream-ошибку ("match_arm_infos отсутствует") — record_
	// generic_method_instantiation по-прежнему явно исключает .Process.

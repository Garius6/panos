...
хендлом (`File_Value.прочитать*`/`Socket_Value.получить*`): воркер там
физически читает `&file.reader`/`&sock.reader`, поле GC-managed объекта, —
безопасно ТОЛЬКО благодаря `gc_pin`/`gc_unpin` (`core/gc.odin`, см.
...
самом объекте (`file_value_native.odin`), блокирующие второе конкурентное
чтение и откладывающие `.закрыть()` до завершения воркера.

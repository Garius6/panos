...
### Альтернатива: регистрация без `pub export fn`/`rdynamic`

`pub export fn` + `rdynamic` — не единственный способ опубликовать
host-функцию. `panos.hostFunctions(...)` регистрирует обычные Zig-функции
напрямую в `Runtime`, без экспозиции символов в таблице динамических
символов исполняемого файла и без прохода через `dlopen`/`dlsym`/libffi на
каждом вызове:

```zig
const panos = @import("panos_embed");

fn game_damage_multiplier(base: f64) f64 {
    return base * 1.5;
}

var runtime = panos.Runtime.init(allocator, .{
    .host_functions = panos.hostFunctions(.{
        .game_damage_multiplier = game_damage_multiplier,
    }),
});
```

Скриптовая сторона идентична предыдущему примеру — синтаксис `внешний
"хост" функ ...` не меняется:
...
набора) — ошибка КОМПИЛЯЦИИ движка в месте вызова `hostFunctions(...)`, не
рантайм-ошибка панос. Несовпадение между зарегистрированной сигнатурой и
`.pns`-декларацией — `Resolve Error` при `Runtime.compile()`, с указанием
имени функции и характера несовпадения.

Два пути можно сочетать в одном приложении; при совпадении имени
регистрация через `hostFunctions` имеет приоритет над `pub export fn`.
`rdynamic` остаётся нужен, только если хотя бы одна host-функция всё ещё
идёт старым путём.

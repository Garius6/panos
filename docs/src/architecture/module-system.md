...
директория `std` (стандартная библиотека, `.pns`-исходники). Первое
...
если `<место>/<спек>.pns` не существует как файл, но `<место>/<спек>/`
существует как директория — пробуется канонический `<место>/<спек>/
индекс.pns` внутри неё (`index.js`/`__init__.py`-конвенция). Чисто
...
в `модули/<имя>/` целиком (`../panosiki/pan/кэш.pns::разложить_запись`):
...
которые НЕ имеют `.pns`-файла на диске (реализованы нативно в VM/компиляторе)
— создаёт `^Module` "на лету", без чтения файла, и регистрирует экспорты
программно (`add_builtin_export`). Файловые модули (`std/*.pns` вроде
...
таких модулей физически нет `.pns`-файла, который можно было бы распарсить.
...
тестовый путь БЕЗ `Module_Graph` не грузит файловые `std/*.pns`-модули с
...
| Новая экспортируемая функция в существующем `std/*.pns`-модуле | см. [рецепт](./recipes/new-stdlib-function.md) |
...
| Изменить, как резолвится путь импорта (конкатенация/суффикс `.pns`) | `resolve_import_path` (`core/resolver.odin:309`) |
| Изменить конвенцию директория-как-пакет (сейчас канонический файл `индекс.pns`) | `resolve_import_dir_index_path` (`core/resolver.odin:332`) |

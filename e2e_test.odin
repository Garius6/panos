...
test_math_and_logic :: proc(t: ^testing.T) {
	...
	tests := []TestCase {
		...
		{
			"Результат: или сохраняет успех",
			`
			функ старт() -> Число
				пер р: Результат(Число, Ошибка) = Успех(42)
				р.или(Успех(7)).получить(0)
			конец
		`,
			f64(42.0),
		},
		{
			"Результат: или возвращает запасной результат",
			`
			функ старт() -> Число
				пер р: Результат(Число, Ошибка) = Неудача(Ошибка("нет", "нет значения"))
				р.или(Успех(42)).получить(0)
			конец
		`,
			f64(42.0),
		},
		...
		{
			"Опция: или сохраняет значение",
			`
			функ старт() -> Число
				пер о: Опция(Число) = Есть(42)
				о.или(Есть(7)).получить(0)
			конец
		`,
			f64(42.0),
		},
		{
			"Опция: или возвращает запасную опцию",
			`
			функ старт() -> Число
				пер о: Опция(Число) = Нет()
				о.или(Есть(42)).получить(0)
			конец
		`,
			f64(42.0),
		},

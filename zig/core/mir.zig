...
pub const Module = struct {
    ...
    // Имя каждой функции верхнего уровня, зарегистрированной как
    // обработчик `DOM.после_кадра` (по строковому литералу-аргументу),
    // собирается один раз во время обхода достижимости
    // tree-shaking в `mir_lowering.zig` (`addDomHandlerRoots`) —

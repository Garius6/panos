...
pub fn nativeModuleExports(name: []const u8) ?[]const []const u8 {
    const table = [_]struct { name: []const u8, exports: []const []const u8 }{
        ...
        .{ .name = "сеть", .exports = &.{ "подключиться", "кодировать_url", "http_запрос", "http_запрос_sync", "http_сервер_слушать" } },
...
const Resolver = struct {
    ...
    fn installBuiltins(self: *Resolver, skip_prelude_hardcode: bool) !void {
        ...
        try self.installBuiltinModule("сеть", &.{ "подключиться", "кодировать_url", "http_запрос", "http_запрос_sync", "http_сервер_слушать" });

...
pub const Vm = struct {
    ...
    fn osVersion(self: *Vm) anyerror!void {
        const heap_string = try self.heap.createString(try self.allocator.dupe(u8, "0.3.3"));
...
test "VM dispatches generic interfaces through vtables" {
    ...
    const source =
        ...
        \\конец
        \\функ старт() -> Число
        \\    пер итератор: Итератор(Число) = Счётчик(42)
        \\    итератор.следующий().получить(0)
        \\конец
    ;
...
test "VM substitutes top-level constants" {
    ...
    const source =
        \\функ число() -> Число
        \\    если ФЛАГ тогда МИНУС иначе 0 конец
        \\конец
        \\функ текст() -> Строка
